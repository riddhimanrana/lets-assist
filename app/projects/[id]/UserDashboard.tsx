...
export default function UserDashboard({ project, user: _user, signups }: Props) {
  ...
  const eventCards = useMemo(() => {
    ...
    return signupStatuses.map((status) => {
      ...
      switch (status.renderState) {
        ...
        case 'hoursPublished':
          return (
            <Card key={status.signup.id} className="border-success/30 bg-success/5 overflow-hidden">
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
                  <div className="bg-success/10 p-2 rounded-full">
                    <Award className="h-5 w-5 text-success" /> {/* Using Award icon */}
                  </div>
                  <div>
                  ...
                  {status.checkInTime && status.checkOutTime && (() => {
                    ...
                    return (
                      <div className="flex justify-between items-center">
                        ...
                        <span className={`font-semibold ${duration.isValid ? 'text-success' : 'text-destructive'}`}>
                          {duration.text}
                        </span>
                      </div>
                    );
        ...
        case 'postEventHours':
          return (
            <Card key={status.signup.id} className="border-warning/30 bg-warning/5 overflow-hidden">
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col space-y-2 text-sm">
                  ...
                  {status.checkInTime && status.checkOutTime && (() => {
                    ...
                    return (
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-muted-foreground">Total Hours:</span>
                        <span className={`font-semibold ${duration.isValid ? 'text-warning' : 'text-destructive'}`}>
                          {duration.text}
                        </span>
                      </div>
                    );
        ...
        case 'missedEvent':
          return (
            <Card key={status.signup.id} className="border-destructive/30 bg-destructive/5 overflow-hidden">
              <CardHeader className="pb-2">
                <div className="flex items-center gap-3">
            ...
            <Card key={status.signup.id} className="border-info/30 bg-info/5">
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-info">{status.timeUntilStartFormatted}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="font-medium text-muted-foreground">Session Time:</span>
                  <span>
  ...
  function getSessionDisplayName(project: Project, startTime: Date | null, details: SlotDetails): string {
