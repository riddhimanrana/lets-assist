...
describe("quarantine reason vocabulary is identical across the route and the SQL", () => {
  ...
  test("quarantine coordinate surrogates satisfy the database's exact bound", () => {
    const eventConstraint = region(
      migrationSql,
      "CONSTRAINT csf_comm_webhook_quarantine_event_id_shape_check",
      "CONSTRAINT csf_comm_webhook_quarantine_body_hash_check",
    );
    const messageConstraint = region(
      migrationSql,
      "CONSTRAINT csf_comm_webhook_quarantine_message_id_check",
      "CONSTRAINT csf_comm_webhook_quarantine_occurrences_check",
    );
    const eventMaximum = Number(
      eventConstraint.match(/char_length\(provider_event_id\) <= (\d+)/)?.[1],
    );
    const messageMaximum = Number(
      messageConstraint.match(
        /char_length\(provider_message_id\) <= (\d+)/,
      )?.[1],
    );
    expect(eventMaximum).toBe(255);
    expect(messageMaximum).toBe(255);

    const raw = `opaque-provider-coordinate-${"x".repeat(1_000)}`;
    const eventCoordinate = route.quarantineCoordinate("provider_event", raw)!;
    const messageCoordinate = route.quarantineCoordinate(
      "provider_message",
      raw,
    )!;

    expect(eventCoordinate.length).toBeLessThanOrEqual(eventMaximum);
    expect(messageCoordinate.length).toBeLessThanOrEqual(messageMaximum);
    expect(eventCoordinate).toMatch(/^qevt_sha256_[0-9a-f]{64}$/);
    expect(messageCoordinate).toMatch(/^qmsg_sha256_[0-9a-f]{64}$/);
    expect(eventCoordinate).not.toContain(raw);
    expect(messageCoordinate).not.toContain(raw);
    expect(route.quarantineCoordinate("provider_event", raw)).toBe(
      eventCoordinate,
    );
  });
