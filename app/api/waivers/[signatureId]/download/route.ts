...
export async function GET(
  ...
  const { data: project, error: projectError } = await adminClient
    .from("projects")
    .select(
      "creator_id, organization_id, can_be_managed_by_staff, waiver_pdf_storage_path, waiver_pdf_url",
    )
  ...
  if (projectError) {
    ...
    console.error(
      "Database query error while loading project in download route:",
      projectError,
    );
    return NextResponse.json(
      { error: "Database query failed" },
  ...
  if (typedProject.organization_id && user) {
    ...
    if (orgMemberError) {
      console.error(
        "Database query error while loading org membership in download route:",
        orgMemberError,
      );
      return NextResponse.json(
        { error: "Database query failed" },
  ...
  if (payload && requiresPdfGeneration(payload)) {
    ...
    try {
      ...
      if (!typedSignature.waiver_definition) {
        return NextResponse.json(
          { error: "Waiver definition not found" },
          { status: 404 },
        );
      }
      ...
      const storageResolver = async (path: string): Promise<ArrayBuffer> => {
        ...
        if (error || !data) {
          throw new Error(
            `Failed to download signature asset from ${bucket}/${path}`,
          );
        }
  ...
  if (typedSignature.signature_storage_path) {
    try {
      if (!hasSourcePdf) {
        ...
        return NextResponse.json(
          { error: "Waiver PDF not found for legacy signature" },
          { status: 404 },
        );
      }
      ...
      const storageResolver = async (path: string): Promise<ArrayBuffer> => {
        ...
        if (error || !data) {
          throw new Error(
            `Failed to download legacy signature from waiver-signatures/${path}`,
          );
        }
  ...
  if (typedSignature.signature_text) {
    try {
      if (!hasSourcePdf) {
        return NextResponse.json(
          { error: "Waiver PDF not found for typed signature" },
          { status: 404 },
        );
      }
