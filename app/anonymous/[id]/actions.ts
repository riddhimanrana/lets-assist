...
export async function startAnonymousGoogleLink(
  ...
  const canonicalPath = `/anonymous/${encodeURIComponent(anonymousId)}?token=${encodeURIComponent(anonymousToken)}`;

  return runOnCanonicalAuthOrigin(canonicalPath, async (origin) => {
