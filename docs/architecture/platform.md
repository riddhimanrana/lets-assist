...
The browser is untrusted. The browser has no access to `plugin_data` or the private CSF implementation: browser roles hold no `plugin_data` schema usage. `private` and `app_private` are omitted from PostgREST's exposed schemas and are secured per object with explicit execution ACLs; reviewed browser reads in `public` remain subject to RLS.
...
| Supabase/Postgres                                        | Enforce RLS for reviewed browser-reachable `public` data; deny browser roles `plugin_data` schema usage; keep `private` and `app_private` outside PostgREST's exposed schemas and apply explicit per-object execution ACLs; preserve tenant foreign keys and constraints; use transaction RPCs to revalidate authority under lock and atomically write state, audit events, outbox entries, and receipts. Privileged clients never make upstream authorization optional. |
...
Supabase/Postgres is authoritative. RLS protects ordinary reviewed access in `public`. Plugin-owned records live in browser-inaccessible `plugin_data`, which remains organization-scoped and server-only through schema and object ACLs. Helper schemas are not PostgREST APIs: every `private` and `app_private` function still requires its own reviewed revoke/grant decision. Consequential multi-record changes use reviewed database transactions/RPCs and immutable audit events.
