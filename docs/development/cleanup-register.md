...
### Production 1.2.31 release and final source corrections

Production host `f742c18df694e4c9e77ea68e2ea9eddc96556c01` deployed through workflow `34652138968`; Vercel deployment `dpl_2oA7zfyPhWjPciasGV1C5ETsuMBq` reached Ready. Forward migration workflow `34652011615` applied the reviewed ledger through `20260911212627`. Child workflow `34652150007` deployed signed application 1.2.31. Normal organization controls selected 1.2.31 for both the real chapter and the marked test workspace. All import and communication worker flags remained disabled.

Chrome verified the test class link through Google sign-in, return to the intended class, and creation of a distinct self-owned fictional profile. The member had no staff controls. The test workspace published policy version 2 allowing outside volunteering. Both member eligibility and a staff-entered fictional point submission refused an account without approved semester membership. No point record or real chapter decision was created. Earlier staff checks also verified role removal and the released seat after testing.

The source has 235 response rows. The old saved range ended at row 172. Reconnecting the same source through normal controls expanded it to row 336 and retained the Fall 2026, grade-derived class mapping. However, the database rejected 197 unchanged committed identities after the mapping version increased. Every prior row hash, identity, contact, submission timestamp, cohort and term remained unchanged. The full preview is held; it is not an accepted import. Existing 200 application coordinates remain unique. The earlier 37-row review count must not be substituted for the new preview's readiness.

Forward migration `20260911223138_csf_application_retry_range_expansion` permits only succeeded committed bindings to survive an increased mapping version with identical immutable mapping semantics and a nonshrinking row end. Pending staff matches remain version-bound. Existing identity, candidate, source, authorization and audit checks remain. Parent review found no blocker; 26 new and 22 existing focused database assertions passed on an isolated local stack. This is not full integrated replay evidence.
...
### September 11 embedded 1.2.34 publication candidate

Private release workflow `34661560730` published signed embedded source `bb7029dda2fafaa107eaad1f55003fce2ac19cab`. Standard root integration workflow `34661582942` generated root PR 539 at `f95f7ff7342a98352ae1c7b9cde90002beaf5e5f` and publication `20260912002546_publish_dvhs_csf_1_2_34`. The root gitlink now matches the signed embedded source. Migration SHA-256 is `369ef5ec515317b8354bc15f56a2c4050829e73f0d3c92bf390f7b838a334e40`; the 498-version ordered-ledger SHA-256 is `26dfd00d401675d971d1c0abce0355b9f703091fac77cdb5440341bd9ee1baa2`.

Publication 498 preserves the reviewed migration 497 schema catalog. It does not update an organization install or apply a Production migration. Production remains at the verified 494-migration checkpoint on host `f742c18d` with private gitlink `6b73d09`. Final root checks, hosted acceptance, copied-workbook acceptance, and Production promotion remain open.

### September 12 Production database release and host credential hold

Final Development candidate `c882b1bb17629b4d2b18049da56ea408799748cc` passed the complete integrated run `34663525023`, including quality, build, replay, SQL authorization/workflow tests, and CSF browser journeys. Hosted functional acceptance passed in `34663522770`. The user waived hosted performance acceptance. Root PR 536 merged as `85bf96a317b740ac533970922a3a190e18161908`, with an identical accepted tree and private gitlink `bb7029dda2fafaa107eaad1f55003fce2ac19cab`.

The final test correction preserves migrated catalog versions in both JavaScript fixture seeding and the local SQL seed. The SQL seed had independently reset the catalog after replay. Its insert fallbacks remain, and fixture installations keep their explicit compatibility baseline. The focused seed suite passed 34 tests and 340 assertions. No applied migration was edited.

Production forward-release run `34664808780` succeeded. Fresh readback confirms 498 migrations, latest `20260912002546`, and catalog version `1.2.34` at signed source `bb7029dda2fafaa107eaad1f55003fce2ac19cab`. The recovered application-range function body MD5 is `b18cf72ece0df071e4f2ee7d93616d06`; execution is allowed for `service_role` and denied to `authenticated` and `anon`. No organization installation was changed. All import, workbook, communications, and scheduled-publishing switches remain off.

Host release run `34664885370` stopped before staging a build: `Verify Production project binding` received HTTP 403 from Vercel. The log does not identify which of the three preflight requests failed, so it does not establish token expiry or revocation. The connected Vercel tool can still read the expected project and team, but that does not prove the workflow credential is valid. Do not deploy through another identity or tool to work around this failure. First diagnose the failing endpoint using the existing Production credential. Replace a credential only if the provider evidence requires it, then dispatch a new accepted app release for root `85bf96a317b740ac533970922a3a190e18161908` and accepted Development `c882b1bb17629b4d2b18049da56ea408799748cc` with the recorded performance waiver. The failed run must not be described as a deployment.

The prior host remains the last deployed application. Authenticated Chrome still renders the test workspace settings after the database update. A separate unauthenticated status request was rate-limited with HTTP 429, so it did not provide new public-version evidence. The new workbook-copy and staff-composer host code is not live. No copied-workbook, application-decision, positive point-cycle, or staff-only audience acceptance was added during this release attempt. There are still zero registered test copies and no live sync destinations enabled. Fresh signup/email recovery still requires the separate test-account handoff. Earlier ambiguous identities and the prepared Home import-count correction remain open.
...
Read-only Production checks confirmed 498 migrations through `20260912002546`, embedded catalog 1.2.34 at `bb7029d`, and zero new sync destinations. There are 200 Fall 2026 applications from the supplied response workbook, no duplicate source coordinates across imported applications, and two legacy account links still pending with matching restriction audit events. Classes 2027–2030 are active; 2024–2026 remain archived.

A bounded timestamp-column read of the live response workbook found 242 responses through row 243, with the latest timestamp September 11 at 7:04 PM Pacific. This supersedes the earlier 235-response source count. The saved 235-row preview remains held and is not a completed reconciliation. Automatic intake expands the reviewed mapping to the live grid row count while preserving its tab, columns, header and identity rules; 41 focused refresh/range tests passed.

Chrome sign-in through the test Class of 2028 link returned the existing independently verified student to Fictional Rollout Student. The test organization still contains exactly two profiles and two verified connections, one self-owned and one staff-verified. No real chapter decision, point award or post was changed. New same-user pending-connection point regressions passed 46 pgTAP assertions for initial submission and resubmission.

Diagnostic PR 543 passed root quality, database and browser checks and merged into Development as `a1ce35e0`. PR 544 releases a read-only diagnostic workflow using the existing Production Vercel credential. It must identify the failing endpoint before any credential replacement is requested. Live sync, outbound email and scheduled publishing remain disabled pending the remaining release and copied-workbook verification.

### September 12 final no-comments review corrections

Root PR 545 contains the 500-migration candidate. Review found and corrected no-comments local-message version drift and missing explicit trigger ACLs. Four organization-leading indexes close the architecture audit finding. The focused no-comments database suite passed 30 assertions. A separate existing source-key index test now uses covered projection columns while retaining the lookup predicates; its four assertions passed. The complete generated release catalog is being checked against an actual replay, including changed table constraints, indexes and snapshot functions. The staff directory's confirmed-login search also needs a scoped read helper because Production service role cannot select `auth.users`; that candidate must pass tests executed as service role before release.

Private review corrections recover uncertain values-only writes, preserve explicit no-comments intent across interrupted destination setup, load reported contacts in paged member rows, label the semester actually submitted, and hide disabled legacy writeback controls. Private PRs 365 and 366 passed and merged. PR 367 contains the remaining setup and roster corrections. No 1.2.35 release tag or new Production host deployment exists at this checkpoint.

Chrome verified that the four real class links resolve to Classes 2027 through 2030 while signed out. The test class invitation dialog exposes its link, code and QR image. A staff application detail page had direct Drive evidence links and no nested scrolling region. The administrator session was restored without joining an additional class. Fresh email-link signup and the copied-workbook application/point journey remain unverified in Production.

Read-only Production checks confirmed all 12 release-worker control rows have workbook refresh, import commit, communications and scheduled publishing disabled. Legacy `CSF_SHEET_WRITEBACK_ENABLED` must also remain unset or non-true to prevent source cell-note dispatch; the new no-comments transport does not override that separate flag. Root PR 546 passed all checks and merged as `fc5cbed3`, restricting the Production-credential diagnostic to reviewed main. PR 544 must pass its updated main release checks before that diagnostic runs. Existing HTTP 403 evidence still does not identify a failed endpoint or establish token expiration.

### September 12 credential diagnosis and atomic setup

Reviewed diagnostic PR 544 merged as `12590ea3c7c02bff50c9c9f52924346def2b02ae`. Run `34669049945` used the existing Production credential and failed at Vercel's project GET endpoint with HTTP 403 and provider code `forbidden`. It never reached the later rolling-release checks. This establishes that the workflow credential cannot access the configured project; it does not establish expiration. The owner must restore project access or enter and save the replacement GitHub Production secret. No credential was created or changed, and no alternate deployment identity was used.

The pending staff search migration now uses a tenant- and profile-scoped owner function for confirmed login identity. Directory RPC signatures and invoker security remain unchanged. Commit `6034e557` integrates the fix; all 46 focused database assertions passed on a fresh 500-migration replay, including calls executed as service role. No broad Auth table grant was added.

Commit `34e53d13` makes destination setup and discussion transport selection atomic in pending migration `20260912015112`. New destinations choose no-comments mode. Omitted intent preserves an existing transport, and failed setup rolls back row creation. The focused database suite passed 37 assertions and its contract suite passed four tests. The matching private action change is integrated locally; final private review and actual 499/500 catalog acceptance remain pending. Neither pending migration has been applied in Production.

The final 499 and 500 generated catalog acceptance queries each returned `1` against separate fresh databases. Catalog commit `e61465cb` includes the atomic setup function, all affected relation digests and ACLs, the snapshot wrapper, the scoped identity helper, and directory functions. The focused catalog suite passed 80 tests. Integrated lint and type checking passed. Private PR 367 passed its complete quality gate at `441a1c3` and merged as `a80eb278`; its setup and roster review findings are resolved. Release publication and Production deployment remain pending.

The held 235-row preview predates the retry-recovery correction already applied in Production. Its stored issues do not prove a current recovery defect. Recovery runs during a fresh preview and does not rewrite old sealed previews. After host deployment, prepare a fresh preview through the saved source. Re-evaluate unchanged source-lineage rows against the current source and leave name-only ambiguities and conflicts for staff. Do not manually modify the old preview or infer account ownership from these matches.

A fresh read-only Production audit found 200 Fall 2026 applications, all pending, with zero duplicate profile/term groups. The latest committed 234-row attempt processed 197 rows and held 37; it had no failed or unknown outcomes. The 235-row held preview remains the latest persisted preview, while the separate live Sheet read found 242 responses. These are different counts and must not be presented as one completed import.

The real organization has two account links, both pending with unknown ownership basis, and zero verified links. No active `csf_profile_restrictions` rows exist; the effective restriction is the pending account status. Inspected snapshot and point functions require a verified exact-profile connection. Pending member snapshots return a null profile. Automatic application profile creation leaves identity emails null, and contact capture writes only reported-contact fields with no account link. This is database definition and aggregate evidence, not a fresh authenticated pending-member browser test.

Private promotion PR 364 merged as `85fc9737fcc41049339163a92877d2605c551814`. Return PR 368 merged as `fbc605e8f9bd0d9fbe07297ef1e6a13701e9aa1c`; both passed private quality and secret checks. The reviewed main source is an ancestor of Development. Publication still awaits the root schema merge. Root review identified a missing catalog entry for the delegated snapshot implementation and explicit owner grants; these must be corrected before merging PR 545.

Commit `c046e3f6` closes both final catalog findings. It adds the delegated snapshot definition/body with owner-only execution to the catalog and explicit postgres grants for both internal helpers. A fresh 500-migration replay accepted the generated catalog query; 39 no-comments database assertions, 51 focused release tests and four contracts passed. Migration `20260912015112` now hashes to `a7f2e083c67fcc2063c7b49ecb5b8ebc012d444b9a6160e4b8a01b12a24ac463`. The SQL change only makes existing owner execution explicit. It does not change data or sync behavior.

### September 12 signed 1.2.35 integration

Root schema PR 545 merged as `60dc95a85115df7089861daae696ca21b4ba681e` after its required quality check passed. All known review findings were resolved. Private tag `dvhs-csf/v1.2.35` points to reviewed main `85fc9737fcc41049339163a92877d2605c551814`, which is also an ancestor of private Development. Signed release run `34670756461` and root integration run `34670778830` succeeded.

Generated PR 547 starts at `af1546a98e8698beff4a9f57473b3d3721b2b741` with the exact signed gitlink, registry entry, publication `20260912033551_publish_dvhs_csf_1_2_35.sql`, and its generated database contract. The strict private-submodule check passed. Publication changes the available plugin catalog, not any organization installation. The 501-version release catalog and final integrated checks remain in progress.

Production still has 498 applied migrations and the prior host release. The GitHub Production Vercel credential cannot read the configured project. The current Chrome Vercel account also does not list the expected Let's Assist team. Credential restoration is a human handoff; no secret was entered, changed or copied. The legacy writeback environment flag could not be independently checked in that browser session. Keep it unset or non-true when deploying. No copied-workbook acceptance or live sync enablement has occurred.

The release staging payload now explicitly sets `CSF_SHEET_WRITEBACK_ENABLED=false` for both build and runtime, alongside the existing paused worker switches. This closes the legacy cell-note dispatch gap for the next deployed host without reading or changing provider secrets. Its six focused stage tests passed. The currently deployed host's flag remains unverified; no live decision or writeback test was performed against it.

The integrated 501 catalog is complete in `fdbb9e32`. The generated publication hashes to `8d8b2c063d638d4479d6c1fed23673566db08a8f0886e0c5c46986f9a18f897c`; the full version ledger hashes to `8bcbd4f9b3f227a586147e978b01e099e3a7eedbaca28e32e45795768f03da6a`. A fresh 501 replay accepted the generated catalog query, all 81 focused catalog tests passed, and 24 signed integration tests passed on the combined checkout. The generated publication, signed registry identity and private gitlink were preserved. Final root CI and deployed acceptance remain open.

The first integrated PR 547 database run found stale serving-catalog assertions in historical release tests. Commit `79ef9d02` updates only their current catalog expectations and adds a preflighted generator helper so future embedded publications refresh those expectations together. Recorded release versions, source identities and digests remain unchanged. No migration or signed publication changed. The corrected full database phase passed all 291 files and 8,163 tests. Its isolated worker lacked an initialized private submodule for the later registry phase; the primary checkout then passed the registry gate, all 25 release integration tests, and the strict exact-gitlink check. Final CI is being rerun on this correction. Production remains unchanged and blocked on the existing workflow credential's Vercel project access.

### September 12 verification while host deployment is blocked

The owner requested all possible local and Production verification while the host release remains blocked. Diagnostic run `34676836302` again returned `endpoint=project status=403 error.code=forbidden` using the existing Production credential. GitHub's configured project and team IDs match the read-only Vercel project result. No configuration typo was found. The Production environment secret shadows the repository secret; neither was read, replaced or removed. No alternate credential or deployment path was used.

CI run `34671837758` passed root quality, all 291 database files and 8,163 assertions, then 92 CSF browser tests. Its single browser failure expected the superseded public join copy in the authenticated member Home card. Commit `0284ae5a` updates that expectation; the compiled focused browser case passed. The historical workbook browser case targets a retired uploader and remains skipped. Its replacement uses user-authorized Google Sheets, which the isolated browser harness does not provide. Three screenshot-gallery cases are opt-in. These skips do not count as passed functional checks.

Chrome verified a staff-connected fictional profile returns through Google sign-in to its correct test class using a login email different from its reported contact. The expired-email recovery page preserves the intended class in login and signup links. It also exposed an unusable resend control when no email was present. Commit `3d801d21` hides that control and its challenge while retaining the continuation links; three focused tests and lint passed. This host fix is not deployed.

Open P1: a Production organization staff account without point-review permission can switch to member view, but opening Point submissions throws `Unauthorized CSF point appeal access` instead of reading its own verified profile. Runtime digest `2178589592` on deployed host `f742c18d` confirms the failure. The signed `85fc9737` source retains the same role mismatch. Fix the member reader scope without widening staff queue permissions, preserve the signed release, and add a browser regression before release.

A fresh read-only real-chapter audit found 1,383 active profiles, four active classes, and 840 active profile/class memberships. No active membership references an inactive profile or places a profile in multiple active classes. All 768 applications have valid profile, class, term and source lineage, with zero duplicate profile/term groups or source coordinates. There are two pending account links and no verified account links. Production has zero point submissions, so empty point/credit mismatch counts do not establish workflow acceptance. All 9,403 successful application import lineage rows retain targets. The current recovery function remains service-only with body MD5 `b18cf72ece0df071e4f2ee7d93616d06`. No Production data was changed.

### September 12 member and correction fixes verified locally

Private PR 369 passed its complete quality run `34677964016` at `e280ea8` and merged into private Development as `1fab5a2267649f20537b44a0e267359095e3a9c9`. It prepares version 1.2.36. It fixes the member-mode point reader mismatch and exposes submitted application corrections to authorized reviewers through the existing review action. The signed 1.2.35 source remains immutable. An independent review of the combined behavior changes found no actionable defects. These fixes are not deployed; the Production P1 remains open until the repaired host is verified.

Three compiled local browser journeys passed on the combined private changes: application correction and separate approval; point submission, correction, resubmission and approval; and an organization staff account reading its own member history without queue permissions. The point cycle preserved proof and review history, kept pending totals unchanged, and created exactly one verified credit after approval. The application cycle retained failed check data and unrelated records while recording the explicit staff decision. Foreign profile history stayed excluded from staff member view.

Focused checks passed: seven point reader tests with 72 assertions, 19 active-membership tests with 193 assertions, 16 correction tests, and five updated submission-scope harness tests. Root browser commits `8999343b` and `6c766c6b` contain the new journeys. They remain separate from the signed 1.2.35 root integration until the new private release is integrated. No real chapter application, point, post, role or account connection changed during these local tests.

The latest Production audit and Chrome checks do not replace the remaining release-dependent acceptance. A fresh import preview, restricted copied-workbook journey, live destination enablement, and Production verification of these fixes still require the corrected host. Imports, live sync, outbound email and scheduled publishing remain paused.
...
### September 12 final signed local candidate

Private promotion PR 370 passed quality run `34678708985`; return PR 371 also passed. Signed tag `dvhs-csf/v1.2.36` points to `20141fb766409d3b9b493128a8cc6c4a6710a20d`, contained in private main and Development. Publication run `34678907707` succeeded and the normal integration workflow opened root PR 549 at `5c70b165`. Root PR 548 is superseded by that integration. No Vercel rollout or Production write occurred.

The final local candidate `7133b23e` combines the signed source, all member/review/auth fixes, and the new browser tests. Its six compiled journeys passed together on a fresh owned 502-migration stack (run `final-integrated-six-502-3`, 1.1 minutes). An earlier combined attempt exposed a test click before staff controls hydrated; the test now waits for the existing page-ready marker. The application behavior was unchanged by that test correction. Local application correction, point correction/resubmission and exact totals, own-profile staff member view, position grant/revoke, staff-only audiences, and real Mailpit expired-link recovery all passed.

The exact signed gitlink check passed. The 502 catalog suite passed 82 focused tests, signed integration passed 25 tests, and the generated publication passed all eight pgTAP assertions against the fresh local database. The actual generated catalog acceptance query returned `1`. Publication `20260912064503_publish_dvhs_csf_1_2_36.sql` hashes to `8a0d5639ae4e28597ee8f0675227b70bad30723200532fe3a931b8126e108cd8`; the 502-version ledger hashes to `8a58d84f5ea6a470e35ae52e151ed0b221f5a09bfe72c1e53f94cf42f01ac971`. Historical schema fingerprints were preserved. Full root CI on the final PR remains the release gate.

Production remains on its earlier host with 498 applied migrations. The earlier read-only audits and Chrome checks stand as recorded, but the new repairs are not live. The existing Vercel Production credential still returns project-access HTTP 403. Keep imports and live Sheets sync paused until the repaired host is deployed, copied-workbook acceptance passes, and a fresh source snapshot is reconciled. Comments, outbound emails and scheduled publishing remain disabled. Ambiguous identities remain pending without imported-history access.
