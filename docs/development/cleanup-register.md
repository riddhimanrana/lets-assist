...
CI run `34671837758` passed root quality, all 291 database files and 8,163 assertions, then 92 CSF browser tests. Its single browser failure expected the superseded public join copy in the authenticated member Home card. Commit `0284ae5a` updates that expectation; the compiled focused browser case passed. The historical workbook browser case remains skipped pending repair, and three screenshot-gallery cases are opt-in. These skips do not count as passed functional checks.
