...
Host release run `34664885370` stopped before staging a build: `Verify Production project binding` received HTTP 403 from Vercel. The workflow's Production credential was rejected during project/rolling-release preflight. The connected Vercel tool can still read the expected project and team, but that does not prove the workflow credential is valid. Do not deploy through another identity or tool to work around this failure. An authorized operator must restore the Production `VERCEL_TOKEN` access, then dispatch a new accepted app release for root `85bf96a317b740ac533970922a3a190e18161908` and accepted Development `c882b1bb17629b4d2b18049da56ea408799748cc` with the recorded performance waiver. The failed run must not be described as a deployment.
