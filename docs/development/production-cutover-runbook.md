...
`20260811001500`. The current repository release candidate has exactly 321
ordered migrations through `20260818092855_atomic_paper_scan_review_row_update`, so the
typed read-only preflight pins an exact 85-migration tail. This count is a
...
provider acceptance, and a fresh full 321-migration replay must all be recorded
...
The 85 pending migrations and their exact application release SHA must be
...
context rather than part of the current 85-migration pending set.
...
321-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T9** run only on the 321 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→321 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **321 rows, head `20260818092855`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 85 pending
...
Production-shaped 236→321 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     321-row target path and pass T1–T9
