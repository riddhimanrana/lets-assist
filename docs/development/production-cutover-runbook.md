...
`20260812152300`; this repository has 282 through `20260813011000`. Production
therefore has exactly 46 pending migrations. `dev.lets-assist.com` still serves
the earlier Ready code whose repository ledger ended at 272 through
`20260812132725`: the external Vercel 100-deployment-per-day project cap
prevented the refreshed deployment. Database parity is not
application-deployment parity, and current database parity is not established,
so the hosted release gate remains open until the exact current schema and code
are deployed and accepted.

The exact pending tail includes
...
`20260813010000_atomic_ai_quota_receipts`, followed by
...
applied to hosted Development. The exact local isolated union replay passed 133
pgTAP files and 5,523 assertions against 282 migrations; hosted acceptance
remains pending.

Pull requests #152, #174, #177, and #179 are merged in current `development`;
#158 and #175 remain open with later or interleaving migrations. The 282-row
target is provisional, and the last migration pull request to merge must
recompute the count, head, and exact tail from the merged tree before any
cutover uses them.
...
The 46 pending migrations and their exact application release SHA must be
...
context rather than part of the current 46-migration pending set.
...
282-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T3** run only on the 282 shape and prove target relations, expected
  ...
  functions, the atomic AI quota receipt index, and removal of `pg_graphql`.
...
ledger is nine migrations behind the repository's 282-migration target. Even
after parity, ordered application against the Development database would not
exercise the repository branch's Production-shaped 236→282 transition,
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **282 rows, head `20260813011000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 46 pending
...
Production-shaped 236→282 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     282-row target path and pass T1–T3
