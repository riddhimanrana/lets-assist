...
`20260811001500`. The current repository release candidate has exactly 308
ordered migrations through `20260817050000_publish_dvhs_csf_1_0_0`, so the
typed read-only preflight pins an exact 72-migration tail. This count is a
...
preview storage, and the source-attested DVHS CSF `1.0.0` publication. Hosted
Development database parity, exact served SHA, role-bound browser acceptance,
provider acceptance, and a fresh full 308-migration replay must all be recorded
...
The 72 pending migrations and their exact application release SHA must be
...
context rather than part of the current 72-migration pending set.
...
308-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T9** run only on the 308 shape and prove target relations, expected
...
`20260816083000` against the Development database; it excludes the twelve
repository-only migrations and
does not prove the repository branch's Production-shaped 236→308 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **308 rows, head `20260817050000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 72 pending
...
Production-shaped 236→308 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     308-row target path and pass T1–T9
