...
`20260812152300`; this repository has 275 through `20260812190442`. Production
therefore has exactly 39 pending migrations. `dev.lets-assist.com` still serves the earlier Ready code
...
ledger is one migration behind the repository and proves only ordered application
through `20260812152300`, not the repository branch's Production-shaped 236→275
transition. It does not exercise
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **275 rows, head `20260812190442`** → it was built by replaying the
