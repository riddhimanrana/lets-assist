...
`20260811001500`. The current repository release candidate has exactly 324
ordered migrations through `20260818150000_bound_feedback_candidate_rotation`, so the
typed read-only preflight pins an exact 88-migration tail. This count is a
...
provider acceptance, and a fresh full 324-migration replay must all be recorded
...
The 87 pending migrations and their exact application release SHA must be
...
context rather than part of the current 88-migration pending set.
...
324-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 324 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→324 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **324 rows, head `20260818150000`** → it was built by replaying the
...
Production-shaped 236→324 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     324-row target path and pass T1–T10
