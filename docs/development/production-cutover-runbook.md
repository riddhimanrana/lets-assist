...
release candidate has exactly 356 ordered migrations through
`20260822224500_lease_historical_plugin_application_runtimes`, so the typed read-only preflight
pins an exact 23-migration tail. This count is a
...
provider acceptance, and a fresh full 356-migration replay must all be recorded
...
The 23 pending migrations and their exact application release SHA must be
...
context rather than part of the current 23-migration pending set.
...
356-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 356 shape and prove target relations, expected
...
333-to-356 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **333 rows, head `20260819050728`** → a genuine current-baseline clone.
     Continue.
   - **356 rows, head `20260822224500`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 23 pending
...
Production-shaped 333-to-356 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     356-row target path and pass T1–T10
