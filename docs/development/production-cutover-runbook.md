...
`20260812152300`; this repository has 276 through `20260813010000`. Production
therefore has exactly 40 pending migrations. `dev.lets-assist.com` still serves
...
The 40 pending migrations and their exact application release SHA must be
...
context rather than part of the current 40-migration pending set.
...
276-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T3** run only on the 276 shape and prove target relations, expected
...
ledger is three migrations behind the repository's 276-migration target. Even
after parity, ordered application against the Development database would not
exercise the repository branch's Production-shaped 236→276 transition,
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **276 rows, head `20260813010000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 40 pending
...
Production-shaped 236→276 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     276-row target path and pass T1–T3
