...
`20260812152300`; this repository has 277 through
`20260813012206_google_cap_effect_fencing`. Production therefore has exactly 41
pending migrations. The four unmerged migrations have not been applied or
deployed in hosted Development. `dev.lets-assist.com` still serves the earlier
...
has been established for the 277-migration repository target, so the hosted
release gate remains open.

The exact unmerged tail is
...
- **T1–T7** run only on the 277 shape and prove target relations, expected
  validated constraints/indexes, removal of `pg_graphql`, the fenced Google CAP
  RPC definitions and ACLs, public function/relation ACLs, and storage posture.
...
Development database; it excludes the four unmerged migrations and does not
prove the repository branch's Production-shaped 236→277 transition. It does
not exercise data-dependent DDL, lock behaviour at Production table sizes, or
Production data.
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **277 rows, head `20260813012206`** → it was built by replaying the
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     277-row target path and pass T1–T7
