...
release candidate has exactly 357 ordered migrations through
`20260822231000_stop_disabled_plugin_application_leases`, so the typed read-only preflight
pins an exact 24-migration tail. This count is a
...
provider acceptance, and a fresh full 357-migration replay must all be recorded
...
The 24 pending migrations and their exact application release SHA must be
...
357-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 357 shape and prove target relations, expected
...
333-to-357 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **333 rows, head `20260819050728`** → a genuine current-baseline clone.
     Continue.
   - **357 rows, head `20260822231000`** → it was built by replaying the
...
Production-shaped 333-to-357 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     357-row target path and pass T1–T10
