...
`20260811001500`. The current repository release candidate has exactly 347
ordered migrations through
`20260822032423_publish_dvhs_csf_1_2_4`, so the typed read-only
preflight pins an exact 111-migration tail. This count is a
...
provider acceptance, and a fresh full 347-migration replay must all be recorded
...
The 111 pending migrations and their exact application release SHA must be
...
context rather than part of the current 111-migration pending set.
...
347-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 347 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→347 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **347 rows, head `20260822032423`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 111 pending
...
Production-shaped 236→347 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     347-row target path and pass T1–T10
