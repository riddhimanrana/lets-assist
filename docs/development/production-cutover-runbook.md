...
release candidate has exactly 442 ordered migrations through
`20260903033000_csf_class_workbook_source_receipt_contract`, so the typed
read-only preflight pins an exact 28-migration tail. This count is a
...
provider acceptance, and a fresh full 442-migration replay must all be recorded
...
context rather than part of the current 28-migration pending set.
...
442-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 442 shape and prove target relations, expected
...
it does not prove the repository branch's Production-shaped 414-to-442
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **414 rows, head `20260829092823`** → a genuine current-baseline clone.
     Continue.
   - **442 rows, head `20260903033000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 28 pending
...
indisvalid` (must be empty); `verify-supabase-migration-parity.mjs`;
   ...
   hosted database or 442-migration repository target);
   and
...
Production-shaped 414-to-442 rehearsal as the authority; the pending set's
...
- Re-run `production-cutover-preflight.sql`; it must select the exact
  442-row target path and pass T1–T10
