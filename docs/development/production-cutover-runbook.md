...
`20260812152300`; this repository candidate has 279 through `20260813011500`.
...
`20260813011500_lock_project_lifecycle_transactions`; none has been applied to
hosted Development.
...
- **T1–T3** run only on the 279 shape and prove target relations, expected
  ...
  receipt index, and removal of `pg_graphql`.
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **279 rows, head `20260813011500`** → it was built by replaying the
...
indisvalid` (must be empty); `verify-supabase-migration-parity.mjs`;
   ...
   Development shape and are comparison evidence, not proof for the 273 hosted
   or 279 candidate shape); and
