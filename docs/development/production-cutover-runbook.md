...
`20260812152300`; this repository has 284 through
`20260813013100_lock_project_lifecycle_transactions`. Production therefore has
exactly 48 pending migrations. The eleven unmerged migrations have not been
applied or
...
has been established for the 284-migration repository target, so the hosted
...
`20260813013000_reconcile_project_lifecycle_boundaries`, and
`20260813013100_lock_project_lifecycle_transactions`; none has been applied to
hosted Development.
...
The 48 pending migrations and their exact application release SHA must be
...
context rather than part of the current 48-migration pending set.
...
284-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T7** run only on the 284 shape and prove target relations, expected
...
Development database; it excludes the eleven unmerged migrations and does not
prove the repository branch's Production-shaped 236→284 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **284 rows, head `20260813013100`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 48 pending
...
Production-shaped 236→284 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     284-row target path and pass T1–T7
