...
`20260811001500`. The current repository release candidate has exactly 350
ordered migrations through
`20260822070000_complete_plugin_application_runtime_compatibility`, so the typed read-only
preflight pins an exact 114-migration tail. This count is a
...
provider acceptance, and a fresh full 350-migration replay must all be recorded
...
The 114 pending migrations and their exact application release SHA must be
...
context rather than part of the current 114-migration pending set.
...
350-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 350 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→350 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **350 rows, head `20260822070000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 114 pending
...
Production-shaped 236→350 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     350-row target path and pass T1–T10
