...
Development has 269 ordered migrations through `20260812104754`. This
repository branch has 271 ordered migrations through `20260812115556`, so it
contains 35 Production-pending migrations and two migrations not yet accepted
...
The 35 pending migrations and their exact application release SHA must be
...
context rather than part of the current 35-migration pending set.
...
repository branch's Production-shaped 236→271 transition. It does not exercise data-dependent DDL,
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **271 rows, head `20260812115556`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 35 pending
