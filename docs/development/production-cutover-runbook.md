...
`20260811001500`. The current repository release candidate has exactly 314
ordered migrations through `20260817132000_restore_multi_date_meeting_permission_rechecks`, so the
typed read-only preflight pins an exact 78-migration tail. This count is a
...
preview storage, the source-attested DVHS CSF `1.0.0` publication, application
...
provider acceptance, and a fresh full 314-migration replay must all be recorded
...
The 78 pending migrations and their exact application release SHA must be
...
context rather than part of the current 78-migration pending set.
...
314-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T9** run only on the 314 shape and prove target relations, expected
...
`20260816083000` against the Development database; it excludes the eighteen
repository-only migrations and
does not prove the repository branch's Production-shaped 236→314 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **314 rows, head `20260817132000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 78 pending
...
Production-shaped 236→314 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     314-row target path and pass T1–T9
