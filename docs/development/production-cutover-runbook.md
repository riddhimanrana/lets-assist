...
release candidate has exactly 358 ordered migrations through
`20260822233000_bound_historical_plugin_runtime_grace`, so the typed read-only preflight
pins an exact 25-migration tail. This count is a
...
provider acceptance, and a fresh full 358-migration replay must all be recorded
...
context rather than part of the current 25-migration pending set.
...
358-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 358 shape and prove target relations, expected
...
333-to-358 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **333 rows, head `20260819050728`** → a genuine current-baseline clone.
     Continue.
   - **358 rows, head `20260822233000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 25 pending
...
Production-shaped 333-to-358 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     358-row target path and pass T1–T10
