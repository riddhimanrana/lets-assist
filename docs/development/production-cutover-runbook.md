...
`20260812152300`, and this repository has 276 through `20260812203500`.
Production therefore has exactly 40 pending migrations. `dev.lets-assist.com`
still serves the earlier Ready code whose repository ledger ended at 272
through `20260812132725`: the external
...
repository branch's Production-shaped 236→276 transition. It does not exercise
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **276 rows, head `20260812203500`** → it was built by replaying the
