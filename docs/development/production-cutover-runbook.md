...
`20260812152300`; this repository has 282 through
`20260813012206_google_cap_effect_fencing`. Production therefore has exactly 46
pending migrations. The nine unmerged migrations have not been applied or
...
has been established for the 282-migration repository target, so the hosted
...
`20260813012206_google_cap_effect_fencing`; none has been applied to hosted
Development.
...
- **T1–T7** run only on the 282 shape and prove target relations, expected
...
Development database; it excludes the nine unmerged migrations and does not
prove the repository branch's Production-shaped 236→282 transition. It does
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **282 rows, head `20260813012206`** → it was built by replaying the
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     282-row target path and pass T1–T7
