...
`20260812152300`; this repository has 287 through
`20260813020000_cancellation_preserves_unknown_delivery_outcomes`. Production
therefore has exactly 51 pending migrations. The fourteen unmerged migrations
have not been applied or deployed in hosted Development. `dev.lets-assist.com`
...
has been established for the 287-migration repository target, so the hosted
...
287-migration target.

Pull requests #152, #158, #174, #177, #179, and #181 are merged in current
`development`; #180 remains open with a later migration. The 287-row target is
...
The 51 pending migrations and their exact application release SHA must be
...
context rather than part of the current 51-migration pending set.
...
287-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T7** run only on the 286 shape and prove target relations, expected
...
Development database; it excludes the fourteen unmerged migrations and does not
prove the repository branch's Production-shaped 236→287 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **287 rows, head `20260813020000`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 51 pending
...
Production-shaped 236→287 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     287-row target path and pass T1–T7
