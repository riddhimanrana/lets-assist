...
`20260811001500`. The current repository release candidate has exactly 320
ordered migrations through `20260818074500_atomic_paper_scan_discard`, so the
typed read-only preflight pins an exact 84-migration tail. This count is a
...
provider acceptance, and a fresh full 320-migration replay must all be recorded
...
context rather than part of the current 84-migration pending set.
...
320-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T9** run only on the 320 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→320 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **320 rows, head `20260818074500`** → it was built by replaying the
...
Production-shaped 236→320 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     320-row target path and pass T1–T9
