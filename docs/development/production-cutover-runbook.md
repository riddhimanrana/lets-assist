...
`20260812152300`; this repository has 274 through `20260813010000`. Production
therefore has exactly 38 pending migrations. `dev.lets-assist.com` still serves
...
274-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T3** run only on the 274 shape and prove target relations, expected
...
ledger is also one migration behind the repository's 274-migration target. Even
after parity, ordered application against the Development database would not
exercise the repository branch's Production-shaped 236→274 transition,
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **274 rows, head `20260813010000`** → it was built by replaying the
...
indisvalid` (must be empty); `verify-supabase-migration-parity.mjs`;
   ...
   Development shape and are comparison evidence, not proof for 273 or 274);
   and
...
Production-shaped 236→274 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     274-row target path and pass T1–T3
