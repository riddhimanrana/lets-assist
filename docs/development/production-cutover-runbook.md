...
`20260812152300`; this repository has 286 through
`20260813013300_close_csf_representative_and_publication_races`. Production
therefore has exactly 50 pending migrations. The thirteen unmerged migrations
...
has been established for the 286-migration repository target, so the hosted
...
`20260813013300_close_csf_representative_and_publication_races`; none has been
...
286-migration target.

Pull requests #152, #158, #174, #177, #179, and #181 are merged in current
`development`; #180 remains open with a later migration. The 286-row target is
...
The 50 pending migrations and their exact application release SHA must be
...
context rather than part of the current 50-migration pending set.
...
286-version target, exits non-zero on a partial or divergent ledger, and checks
...
Development database; it excludes the thirteen unmerged migrations and does not
prove the repository branch's Production-shaped 236→286 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **286 rows, head `20260813013300`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 50 pending
...
Production-shaped 236→286 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     286-row target path and pass T1–T7
