...
`20260811001500`. The current repository release candidate has exactly 346
ordered migrations through
`20260822000923_publish_dvhs_csf_1_2_3`, so the typed read-only
preflight pins an exact 110-migration tail. This count is a
...
provider acceptance, and a fresh full 346-migration replay must all be recorded
...
The 110 pending migrations and their exact application release SHA must be
...
context rather than part of the current 110-migration pending set.
...
346-version target, exits non-zero on a partial or divergent ledger, and checks
...
- **T1–T10** run only on the 346 shape and prove target relations, expected
...
does not prove the repository branch's Production-shaped 236→346 transition. It does not
...
3. **Verify it is a clone, not a replay** — `list_migrations` on the new ref.
   - **236 rows, head `20260811001500`** → a genuine current-baseline clone.
     Continue.
   - **346 rows, head `20260822000923`** → it was built by replaying the
...
6. Push, and time it:
   ...
   supabase db push --linked --dry-run      # expect exactly 110 pending
...
Production-shaped 236→346 rehearsal as the authority; the pending set's
...
9. **Post-push verification**, in order:
   ...
   - Re-run `production-cutover-preflight.sql`; it must select the exact
     346-row target path and pass T1–T10
