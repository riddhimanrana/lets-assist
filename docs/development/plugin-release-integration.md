...
2. Update that plugin's runtime manifest version, `release.json`, and changelog. The supported install range must still include the version currently served by the root registry.
...
8. The root workflow pins the exact private commit, updates the code-owned release registry, generates one forward publication migration, and opens a pull request against root `development`.
...
- Vercel builds an embedded plugin as part of the host preview. A later `application` profile may have its own Vercel project, but it must also have direct-access protection and server-side authorization. A separate project is not required for embedded CSF releases.
