...
- The repository candidate has 328 ordered migrations through
  `20260818223637_complete_remaining_function_acl_restoration`. The thirty-two migrations
  ...
  `20260818223637_complete_remaining_function_acl_restoration`).
- Hosted Development serves exact root merge SHA
  `ac99d706f95229d19265e6938df0988dd630473c` and is healthy and
  migration-current at 327 rows through
  `20260818180000_complete_function_acl_restoration`. Google OAuth is
  enabled and the hosted authorize endpoint redirects to Google. The new 328th
  remaining owner-internal function-ACL restoration remains repository-only until this change
  passes CI and is promoted through Development.
- Production remains untouched at the audited 236-row baseline through
  `20260811001500`; the exact read-only preflight now expects a 92-migration
  cutover to this 328-row candidate. Running that cutover requires
...
- Local evidence on the prior candidate includes a fresh exact 324-migration replay
  ...
  hosted replay. The candidate has 166 pgTAP files with 5,954 assertions plus
