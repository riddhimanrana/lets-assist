...
V17: CSF profile auto-link → authenticated verified email exact match only; name-only auto-connect ⊥; submitted names are officer review context.
...
V55: CSF student onboarding → one permanent 6-char class join code per class (32-letter alphabet, no 0/O/1/I; regenerate invalidates, disable withdraws); reusable/student-specific onboarding links, direct invitations, and profile-claim confirmation retired 2026-08-23 (B93); code state separate from email telemetry; join + account/profile/cohort-membership/audit transition atomic and never term-activating.
...
V85: CSF class-code join → verified account email is the only automatic signal: one active same-class email match connects, zero matches create a new profile from the account identity, conflicting account/cohort or email shared by several records → per-class Needs attention officer review; officer Connect requires canonical evidence (confirmed account email = roster email + exact name + one matching active class); names never auto-link.
