...
V17: CSF profile connection → authenticated exact verified-email match, or one member-confirmed passive account-name candidate bound by a signed snapshot and revalidated under lock as the sole active unclaimed record with one active membership in the selected class; manually submitted names remain officer-review context and never create or link a profile.
...
V31: Google Forms/Sheets/Drive → intake, evidence, and migration only after cutover; reviewed platform records authoritative; the single carve-out is decision write-back to the source application sheet (row background color + first-cell note), ledger-recorded, env-gated (`CSF_SHEET_WRITEBACK_ENABLED`), and never a value/report write; all other Google Sheet writeback/report destinations, silent overwrite, and permanent dual authority ⊥; report export = permission-checked local ZIP of formula-safe CSV + manifest.
...
V55: CSF student onboarding → one permanent 6-char class join code per class (32-letter alphabet, no 0/O/1/I; regenerate invalidates, disable withdraws); reusable/student-specific onboarding links and direct invitations retired 2026-08-23 (B93); code state separate from email telemetry; exact-email or signed passive account-name confirmation + account/profile/cohort-membership/audit transition atomic and never term-activating; manually typed names create at most one officer request.
...
V112: CSF Google Sheet + report boundary → Sheets are explicit read/import evidence plus the single decision write-back carve-out: application decisions may write row background color + a first-cell note back to the source sheet, only through the ledger-driven dispatcher, only when `CSF_SHEET_WRITEBACK_ENABLED` is true; value writes, timestamped compatibility tabs, and report destinations stay ⊥; export = permission-checked local formula-safe ZIP + manifest.
