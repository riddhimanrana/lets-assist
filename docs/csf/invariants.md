...
V55: CSF student onboarding → one permanent 6-char class join code per class (32-letter alphabet, no 0/O/1/I; regenerate invalidates, disable withdraws); reusable/student-specific onboarding links and direct invitations retired 2026-08-23 (B93); code state separate from email telemetry; exact-email account/profile/cohort-membership/audit transition atomic and never term-activating; a typed or account name connects only the one unclaimed same-class record it unambiguously matches (exact, recorded nickname, or ≥3-character first-name prefix on an exact last name), recorded as self-confirmed and revocable; every other name outcome creates at most one officer request (Amendment 7, 2026-09-15).
