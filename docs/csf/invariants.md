...
V17: CSF profile connection → authenticated exact verified-email match only; a member-confirmed passive account-name candidate and every manually submitted name remain officer-review context, create or reuse one request, and never create or link a profile.
...
V31: Google Forms/Sheets/Drive → intake, evidence, and explicitly configured export channels; reviewed platform records remain authoritative. Restricted application, point-submission, and named class/semester destinations follow V112. Legacy source-decision colors and first-cell notes retain their own ledger and `CSF_SHEET_WRITEBACK_ENABLED` gate. Unconfigured value/report destinations, silent overwrite, Sheet-based account ownership, and dual authority ⊥; permission-checked local ZIP reports remain available.
...
V49: CSF import-row reconciliation and meeting commit → one organization-scoped server-only database transaction including normalized records, source/job state, explicit actor/reason/correlation, and immutable audit event; retry of the same committed snapshot is idempotent.
...
V51: CSF list query → URL-backed server search/filter/sort + cursor paging; default ≤50, max ≤100; search resets the cursor and preserves the surrounding filters, sort, and selected view; client-side full-dataset filtering ⊥.
...
V53: CSF Google source workflow → connection identity/health + immutable file id/provider version + explicit tab/range/header + stable indexed mapping + normalized row preview + reconciliation + commit summary/retry; Picker cancel ≠ error; title-only matching and silent first-tab/default-range import ⊥.
...
V55: CSF student onboarding → one permanent 6-char class join code per class (32-letter alphabet, no 0/O/1/I; regenerate invalidates, disable withdraws); reusable/student-specific onboarding links and direct invitations retired 2026-08-23 (B93); code state separate from email telemetry; exact-email account/profile/cohort-membership/audit transition atomic and never term-activating; signed passive and manually typed name confirmations create at most one officer request and never link.
...
V60: CSF Google/private-data verification → real Drive sources may be read through bounded protected previews in a temporary local organization or explicitly confirmed hosted Development, with traces and screenshots disabled; real chapter rows never commit outside separately authorized Production; repository artifacts contain synthetic identities only and pass PII/credential scan.
...
V69: mixed-grade application source + reviewed preview rows resolved to configured cohort-term targets → one chapter-wide source and one immutable preview, with each row deriving and committing through its own immutable resolved cohort; source-level fixed class, stale fixed-class scope, unconfigured cohort-term targets, and changed row targets ⊥.
...
V71: CSF Sheet source discriminator → canonical `source_type` equals populated compatibility `settings.sourceKind`; meeting source create/refresh writes the explicit type; contextual sources leaking into class-history workflows ⊥. (partner half retired 2026-08-17, B92)
V72: selected Google Sheet tab + explicit or unqualified A1 range → one canonical range scoped to that same selected tab across inspection, saved mappings, and meetings; mismatched or malformed explicit tab provenance ⊥. (partner half retired 2026-08-17, B92)
...
V84: CSF imported evidence → immutable provider/file id + revision/modified time + content hash + explicit populated tab/range + term + schema/importer version + sensitivity; a Drive title or filename pattern is display evidence only and never identifies the authoritative source; grid capacity, hidden/filter state, filename-derived term, password fields, irrelevant qualitative responses, macros, external links, and rejected columns cannot become authoritative rows or retained logs.
V85: CSF class-code join → one active same-class verified-email match connects; otherwise the server may offer one passive account-name candidate only when it is the sole active unclaimed exact-name profile with exactly one active membership in the selected class. Member confirmation uses a short-lived signed snapshot and a locked database recheck to create or reuse exactly one per-class Record connections request. Changed, ambiguous, claimed, multi-class, or manually entered name evidence follows the same review boundary; no name-only path creates a profile or semester membership.
V86: retired 2026-08-17 (B92) — partner representative access removed entirely; no external club representative accounts, capabilities, or UI exist.
V87: CSF partner-club term lifecycle → append-only term events; current standing/policy-review state is a projection, not an overwritten omnibus status. (audit-operation event kinds retired 2026-08-17, B92)
...
V100: retired 2026-08-17 (B92) — partner representative mutations removed with representative access; the RPCs no longer exist.
...
V102: CSF partner-club term standing → stable request id + database permission recheck + organization lock; current projection, append-only term event, and staff audit commit atomically; exact replay is idempotent, conflicting replay ⊥, and a no-op request records an explicit non-mutating receipt.
...
V106: retired 2026-08-23 (B93) — direct-invitation links removed; the class join code carries no send telemetry, and sent/resend/delivery language still requires a durable recipient delivery receipt.
...
V112: CSF Sheet export → disabled-by-default destination + stable internal record IDs + versioned snapshots + durable write ledger + fresh staff/export authority, privacy, and lease checks. Only managed columns may change; account connection, application status, enrollment, verified points, and completion stay separate. Inbound Sheet decisions require staff review through existing authorized actions; comments never approve, award points, or connect accounts. New destinations default to no comment transport. They keep discussions in Let's Assist, exclude discussion-only changes from Sheet source versions, and neither read nor write native comments or a Comments column. Existing native cell-attached threads and existing Comments-column destinations remain supported. Exported discussion history stays intact, and Sheet edits require staff review before appending a labeled Google Sheets note. A persisted exported-cell baseline distinguishes external edits from newer app messages; neither comments nor their authors establish ownership or decision authority. Ambiguous writes stay held for reconciliation; unverified retries, name-only bindings, unconfigured compatibility tabs, and direct Sheet authority ⊥. Test copies must pass before live enablement. The no-comment acceptance journey skips comment receipts and readback only; it still requires copy lineage, privacy, provider versions, decisions, point outcomes, class output, and repeated export receipts. Local report export remains a permission-checked formula-safe ZIP with manifest.
V113: retired 2026-08-23 (B93) — student-specific secure links removed with the onboarding-link system; recording a current unique roster email still precedes any officer connection evidence.
...
V123: Google Picker + `drive.file` → `setAppId` receives numeric Cloud project number derived server-side from strictly valid `GOOGLE_CLIENT_ID` before `build`; token DTO exposes only access token + numeric app ID, every Picker sets exact page origin, malformed config fails closed with admin recovery, scope remains `drive.file`, and metadata failures log no provider body or file/client identifier.
V124: CSF profile merge → every exact-current-schema reference to `csf_profiles` is cataloged as same-transaction current-ownership rewrite, deliberate immutable-history retention, or canonical preflight blocker; preview and locked execution share every uniqueness rule, including the active point-claim partial index; one organization identity advisory lock precedes request/table/row locks across merge, profile write/import, class-code join, and connection resolution; cross-organization work remains independent; success requires zero unintended live source references plus non-PII review/audit evidence.
V125: CSF profile merge × sheet-import target → only `commit_outcome_state=not_started ∧ commit_frozen_at=NULL ∧ commit_target_profile_id=NULL` is mutable current match ownership; coherent `succeeded ∧ import_status∈{created,updated}` and `failed ∧ import_status=skipped ∧ commit_outcome_resolution=terminally_skipped` lineage retains both matched/frozen source references as immutable commit/recovery evidence; every frozen, in-flight, unknown, historical-unknown, retryable-failed, or malformed remainder ⊥ through one canonical preview/execution blocker under the organization identity lock; claim takes that lock before coordinate/row locks; execution locks affected import rows before preview, rewrites only the mutable class, audits its exact count, and proves zero nonhistorical source references.
V126: CSF preview seal → caller summary contains display metadata only; row-state counts, readiness, status, and seal time are derived from the stored preview rows by the database; a failed seal closes the opened preview once, while an explicit retry opens new lineage instead of reusing the failed preview.
V127: CSF post follow-up add/delete → one service-only organization-scoped database transaction under the staff-access lock rechecks `manage_posts`, binds the reply to a same-tenant published parent, commits the row mutation with one immutable request receipt, returns an exact retry idempotently, and rejects conflicting or stale replay; direct service-role writes and parent-delete cascade ⊥; plugin teardown removes replies before parent announcements.
V128: CSF activity/partner-club staff-authority mutation → each of `csf_create_activity`, `csf_update_activity`, `csf_set_activity_status`, `csf_link_activity_project`, `csf_set_partner_club_status`, `csf_set_partner_club_term_status`, and `csf_upsert_partner_club_policy` keeps its exact prior signature as a service-only fixed-path wrapper that checks the exact permission, takes `csf_staff_access_lock_key(p_organization_id)`, holds the actor's active `organization_members` row `FOR SHARE`, rechecks that permission, and only then enters the owner-only implementation and its request/business locks; a queued call whose role edit, staff-position revocation, host-membership deactivation, or host-membership deletion committed first ⊥ with zero target and zero receipt writes; replay lookup lives inside the implementation, so an exact committed replay by a since-deprivileged actor ⊥ while its earlier committed outcome stays durable and unmodified; direct client or `service_role` execution of any implementation ⊥.
...
T43|~~|close the lifecycle audit defects across identity/application safety, truthful invitations/imports/posts, policy values, provider identity/recovery, point authority, role reachability, action state, member-payload scoping, and accessibility|V31,V32,V50,V52,V55,V59,V61,V66,V74,V78,V85,V89,V92,V104,V105,V106,V107,V108,V109,V110,V111,V112,V113,V114,V115,V116,V117,V118,V119,V120,V121,V124,V125,I.schema,I.service,I.route,I.cmd
T44|x|bind root + CSF Google Pickers to numeric Cloud app ID without widening `drive.file`|V53,V60,V91,V92,V118,V123,I.service,I.route,I.cmd
...
B85|2026-08-11|Picker returned a native Sheet under `drive.file` without linking selection to the Drive app because both builders omitted required numeric Cloud app ID|V123
B86|2026-08-11|clean-worktree production prerender lacked required public Supabase URL/key|rerun build with non-routable synthetic build-only values
B87|2026-08-11|the duplicate-profile merge delegated reference rewrites to a July function that predated later profile FKs, omitted active point-claim collisions from preview, and used global account/cohort locks that serialized unrelated chapters and could deadlock with identity edits/claims|V124
B88|2026-08-11|the merge catalog labeled every sheet-import match mutable while the import freeze trigger pins every reviewed target, so a preview-ready merge delegated a blanket rewrite that failed with `55000` and had no frozen-row regression fixture|V125
B89|2026-08-12|the preview caller passed its parsed `rows` aggregate as display metadata even though the seal RPC derives that key from stored rows, so the database correctly refused the hosted preview after all 85 rows were appended|V126
B90|2026-08-12|private reply actions landed without their root transaction migration, so a clean replay omitted the required RPC and the optimistic browser reply could not persist|V127
B91|2026-08-12|seven service-only CSF activity and partner-club transactions carrying `manage_opportunities` or `manage_partner_clubs` checked mutable staff permission before their request and business locks, and the partner-club standing and policy transactions serialized only on their own per-club-term or per-organization advisory keys, so a queued call could commit after a role edit, position revocation, or host-membership change removed that authority|V128
B92|2026-08-17|partner-clubs simplification (`20260817120000`): representative access, immutable member-Sheet audit-import batches with provenance acknowledgement, and per-club point policy were removed; clubs keep canonical identity + aliases + per-term standing with a reference-only `spreadsheet_url` never read programmatically, apply through form-response export imports whose immutable rows are applied per-row as draft club records or skipped, and member claims are gated only on active standing for the current term with officers vetting caps/types/proof manually at approval; `csf_upsert_partner_club_policy` keeps its name and V128 lock order with a simplified body|V23,V49,V71,V72,V86,V87,V100,V102,V128
B93|2026-08-23|onboarding-link retirement (`20260823210000`–`20260823212000`): reusable class links, student-specific direct invitations, profile-claim confirm/decline, the org-level account-connections view, and student record search were removed; one permanent 6-char class join code per class (regenerate/disable, `/connect/<code>`, `csf_join_class_by_code`) with verified-email-only automatic matching replaces them, unresolved joins land in the per-class Needs attention queue with evidence-gated Resolve, and the per-term `application_form_url` drives the public apply CTA inside the application window|V17,V55,V85,V104,V106,V113,V116,V124
