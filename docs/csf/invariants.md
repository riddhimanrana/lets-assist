...
V122: CSF communications scheduler → service-only durable tenant-fairness state and per-tenant cyclic campaign cursors derive a bounded scope from available queued work ∪ every expired processing lease regardless of campaign status, and terminalization-check a bounded fair campaign scope before/after worker passes; settled campaigns cannot remain open after a lost response, cancelled leases still reach unknown-outcome review without resend, nonterminal/faulting prefixes cannot starve later tenants or campaigns, and live/retry/unknown/undispatched work remains nonterminal.
