...
**Current status:** this repository carries 329 ordered migrations through `20260818232541_complete_final_owner_internal_function_acls`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 93-migration cutover. Hosted Development serves root merge SHA `dafe509ec6775502fc357db6a4990b1592ef07ee` with the verified 328-migration ledger through `20260818223637`, and Google authentication redirects successfully there. The exact 329-migration hosted CI replay is the final candidate gate; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the ACL-complete release candidate, then prove branch-scoped non-production Supabase parity against the 329-row ordered ledger through `20260818232541`
