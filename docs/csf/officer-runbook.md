...
**Release ledger:** the current repository candidate carries 466 ordered migrations through `20260908090508_csf_sheet_automatic_update_authorization`; the private Development gitlink is `ce8d607`. Production last passed the 460-migration release at `82ab06b6`. Development has 462 applied migrations. The four later candidate migrations remain unapplied. Scheduling retirement is deployed to Development at `54c95cf6`, but has not been deployed to Production. The requirement-evidence migration passed CI and reached the Development database. It has not reached Production. The integrated import fixes still require hosted acceptance before release.
