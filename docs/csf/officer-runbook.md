...
**Release ledger:** the current repository candidate carries 497 ordered migrations through `20260911231213_csf_application_retry_canonical_ranges`. Its private release gitlink is `6b73d09`, restored as the serving fallback; the signed application 1.2.32 source remains `a9f528e`. Migration 497 has catalog integration and focused checks; final integrated acceptance remains outstanding. Production remains at 494 migrations, host `f742c18d`, and selected application 1.2.31. Sheet sync remains disabled pending copied-workbook acceptance. See the cleanup register for deployed versions and checks.
...
- New destinations use a Comments column for discussion history and author attribution. Staff review edited cells before adding them to the local history, labeled as Google Sheets edits. Comment text never approves an application or awards points. Existing native-thread destinations keep their format.
