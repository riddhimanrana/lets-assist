...
**Current status:** this repository carries 330 ordered migrations through `20260819002500_serialize_plugin_deletion_and_token_feedback`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 94-migration cutover. Hosted Development serves root merge SHA `42424972d803fd907676b739e9ab8bd59daa1184` with the verified 329-migration ledger through `20260818232541`, and Google authentication redirects successfully there. The exact 330-migration hosted CI replay is the final candidate gate; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the race-guard-complete release candidate, then prove branch-scoped non-production Supabase parity against the 330-row ordered ledger through `20260819002500`
