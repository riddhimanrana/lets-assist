...
**Current status:** this repository carries 324 ordered migrations through `20260818150000_bound_feedback_candidate_rotation`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 88-migration cutover. Hosted Development serves root merge SHA `1b88d77ed3ea2cf6cc2975e744869281f4bd9b4e` with the verified 323-migration ledger through `20260818134000`, and Google authentication redirects successfully there. The fresh full 324-migration local replay is the final candidate gate; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the bounded feedback candidate rotation, then prove branch-scoped non-production Supabase parity against the 324-row ordered ledger through `20260818150000`
