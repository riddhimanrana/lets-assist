...
**Current status:** `origin/development` at `e0e6461` contains 273 migration files through `20260812152300_atomic_csf_post_replies`, 122 pgTAP files, and committed private gitlink `ca817bf`. Private `origin/development` is intentionally four commits ahead at `4f20fa5`; PR #153 does not update the root gitlink. Local run `20260812-pr153-db-validate-smoke` ran `bun run db:validate`: this lint-plus-replay smoke replayed all 273 migrations successfully, but it did not run the 122-file pgTAP suite, architecture audits, advisors, or effective RLS/policy/grant assertions. The dated hosted Development record reports the same 273-version database ledger and a stable alias still serving earlier Ready SHA `cf330e5f`, whose tree ended at 272 migrations after a deployment-cap block. This documentation reconciliation did not re-query hosted Development. Historical Development evidence includes leaked-password protection, removal of unused `pg_graphql`, Maps/project creation, role-aware Help, and controlled email/webhook reduction. Exact-current deployment, current advisors, hosted enabled scheduled-post publication, the remaining Google/import and visible mutation journeys, accessibility, Production webhook rotation, and Production cutover remain pending. Production was excluded from this reconciliation.
...
2. The **Also send this as an email** toggle requests exactly one campaign per post through the durable ledger — retries are safe. A class campaign freezes the current term, exact class cohort, consent topic, content, and recipient snapshot; later member/class changes or post edits do not rewrite it. The result must separately say **Post published** and either **Email queued** or **Email not queued**. A queue failure never means that a persisted post was not saved. `.github/workflows/csf-communications-dispatch.yml` exists on `origin/main`, requests ten-minute runs, selects the GitHub environment named `production`, and invokes the auth-first, bounded, input-free, exact-opt-in route at source-visible path `/api/cron/csf-communications-dispatch`. Only the `CRON_BASE_URL` host is secret-held, so source cannot identify the target deployment. This proves repository schedule ownership, not current hosted behavior. The dated release record reports irregular HTTP 200 starts; it does not establish a Development cadence, a fixed-time SLA, provider delivery, or a Production application result. **Email queued** is not **Email delivered**.
...
6. **Schedule post** is offered only when the target environment has the publisher explicitly enabled. The transition, fail-closed hold states, authenticated route, pgTAP suite, central replay, and GitHub scheduler are implemented, but `.github/workflows/csf-scheduled-post-publisher.yml` remains absent from `origin/main`. A stored due time is still not proof that a particular hosted post entered Feed. Use manual publication until the workflow reaches the default branch and the target environment has a successful enabled worker run plus visible schedule → Feed acceptance. Scheduled posts never queue email; publish now first if email is required.
...
- [x] August 6 `20260806-post-cleanup` companion replay: 214 migrations, 82 CSF tables, 63 pgTAP files, and 3,165/3,165 assertions; see the [verification summary](evidence/20260806-post-cleanup/verification-summary.md). This is not a fresh 273-migration/122-file pgTAP run.
...
- [x] Dated hosted evidence confirms that a persistent Supabase Development branch exists; PR #153 did not re-query it
- [ ] Verify the required explicit `$0.01344/hour` cost authorization provenance; branch existence does not prove that this precondition was satisfied before creation
- [x] Dated hosted Development record reports parity at 273 ordered migrations through `20260812152300_atomic_csf_post_replies`
...
- [x] Historical combined replay/build/static/DV-browser/CSF-browser gates for the exact August 11 root + private-submodule tree
...
- [ ] Configure and repeatedly verify `csf-communications-dispatch` for the environment officers will rely on. The route path is source-visible, but the `CRON_BASE_URL` host is resolved from the GitHub environment named `production`; source cannot attribute the target deployment. Historical runs do not prove Development behavior, Production application behavior, a fixed-time SLA, or provider delivery.

No live chapter Google write has been performed. The dated hosted record reports a separate Development Supabase project with 273-migration ledger parity, but the stable alias still serves the earlier 272-migration code tree. Hosted Development and Production were not inspected or mutated by this reconciliation. Vela was not accessed or mutated. The remaining items are action-time release gates, not completed runbook steps.
