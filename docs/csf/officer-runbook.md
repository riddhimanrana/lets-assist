...
**Release ledger:** the current repository candidate carries 468 ordered migrations through `20260908141739_csf_workbook_matching_tab_authorization`; the private Development gitlink is `d9d227f`. Production last passed the 460-migration release at `82ab06b6`. Development has 462 applied migrations. The six later candidate migrations remain unapplied. Scheduling retirement is deployed to Development at `54c95cf6`, but has not been deployed to Production. The requirement-evidence migration passed CI and reached the Development database. It has not reached Production. The integrated import fixes still require hosted acceptance before release.
