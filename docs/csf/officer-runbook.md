...
**Current status:** this repository carries 320 ordered migrations through `20260818074500_atomic_paper_scan_discard`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 84-migration cutover. Hosted Development serves root merge SHA `10facf97b24fb007a5fa7435796b43ff7d63c32b` with the verified 319-migration ledger, and Google authentication redirects successfully there. The fresh full 320-migration local replay passes; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the atomic paper-scan discard migration, then prove branch-scoped non-production Supabase parity against the 320-row ordered ledger through `20260818074500`
