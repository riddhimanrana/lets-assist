...
**Release ledger:** the current repository candidate carries 482 ordered migrations through `20260910090800_csf_returning_account_revoked_history`; the private release gitlink is `01a188a`. Production has all 482 migrations and serves ownership correction release `bd49f7be`. The newer submission and Home interface changes remain a release candidate. Deployment receipts identify the served application separately from this database ledger.
