...
**Release ledger:** the current repository candidate carries 484 ordered migrations through `20260911101007_publish_dvhs_csf_1_2_25`. The private release gitlink is `bb819ef`, which also supplies the signed application 1.2.26 source. Its publication migration is pending root integration. Production has 482 migrations and serves usability release `84087a03`. The new Sheet sync and current Home import counts remain a release candidate. Deployment receipts identify the served application separately from this database ledger.
