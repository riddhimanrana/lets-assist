...
**Release ledger:** the current repository candidate carries 506 ordered migrations through `20260913053412_publish_dvhs_csf_1_2_38`. Its committed private release gitlink is `3fbabfb`, the signed embedded 1.2.38 source. Publication 506 preserves the reviewed 505 schema. Migration 504 follows reviewed profile-merge lineage when reusing settled class-history source keys; migration 505 allows a published activity to omit all dates while retaining the date-order rules. The 1.2.38 candidate still needs release verification and copied-workbook acceptance. See the cleanup register for deployed versions, worker controls and dated checks.
