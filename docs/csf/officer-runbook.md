...
**Release ledger:** the current repository candidate carries 488 ordered migrations through `20260911192954_publish_dvhs_csf_1_2_28`. The private release gitlink is `d94bcf2`. Signed application 1.2.28 adds the Comments column; its generated publication migration awaits Production application. Production still has 482 migrations and serves usability release `84087a03`. New Sheet sync and current Home import counts remain a release candidate. Deployment receipts identify the served application separately from this database ledger.
