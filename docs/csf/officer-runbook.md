...
**Release ledger:** the current repository candidate carries 491 ordered migrations through `20260911203901_csf_sheet_sync_observation_guard`. The private release gitlink is `6b73d09`. Application candidate 1.2.31 requires a complete inbound Sheet observation before staff can accept a proposed change. Production still has 482 migrations and serves usability release `84087a03`. New Sheet sync and current Home import counts await deployment and final browser checks. See the cleanup register for deployed versions and checks.
