...
**Current status:** this candidate carries 346 ordered migrations through `20260822000923_publish_dvhs_csf_1_2_3`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 110-migration cutover. Hosted Development has the verified 342-migration ledger through `20260821044815`, and Google authentication redirects successfully there. The signed 1.2.3 release is published at zero rollout with separate Preview-compatible Development and Production child artifacts. Browser/provider acceptance remains a release gate. Production email-webhook proof also remains open. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched until the authorized release workflow applies the reviewed cutover.
