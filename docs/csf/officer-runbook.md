...
**Release ledger:** the current repository candidate carries 500 ordered migrations through `20260912015608_csf_member_directory_staff_identity_search`. Its committed private release gitlink is `bb7029d`, the signed embedded 1.2.34 source. The candidate adds no-comments sync and staff contact search. Production has 498 migrations through `20260912002546_publish_dvhs_csf_1_2_34`, host `f742c18d`, and selected application 1.2.31. Sheet sync remains disabled pending the release and copied-workbook acceptance. See the cleanup register for deployed versions and checks.
