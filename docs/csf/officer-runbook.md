...
**Current status:** this repository carries 321 ordered migrations through `20260818092855_atomic_paper_scan_review_row_update`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 85-migration cutover. Hosted Development serves root merge SHA `0e58cd0084027aec8f949e9940bc691e9b41dcf7` with the verified 320-migration ledger, and Google authentication redirects successfully there. The fresh full 321-migration local replay passes; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the atomic paper-scan review-row migration, then prove branch-scoped non-production Supabase parity against the 321-row ordered ledger through `20260818092855`
