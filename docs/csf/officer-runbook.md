...
**Current status:** the last verified public app is `1fbf92268b729250f73fa09523673544a7d25b3e`. All four class workbooks are linked. Preparation does not mean their rows have committed. Workbook refresh and import processing are enabled; communications remains disabled. Production contains 626 imported applications with pending officer decisions. Identity reconciliation and Production delivery proof remain unfinished. Use the [cleanup register](../development/cleanup-register.md) for release and data evidence.
**Release ledger:** the current repository candidate carries 470 ordered migrations through `20260909090944_csf_application_retry_match_recovery`; the private Development gitlink is `ba8030f`. Hosted run `34331389754` passed on Development `cb3e18d8`, but the live application retry failed. Private repair `d91e296` passed CI and merged; root integration, database replay, and live acceptance remain pending. Production has the exact 468-migration ledger and serves `1fbf9226` after app-only run `34315876545`. Scheduling retirement and the earlier import migrations are applied. The new retry repair has not deployed.
The authorized chapter-wide application imports include historical semesters and alumni. The original Class of 2027 through 2030 workbook seed exclusions below do not exclude alumni applications. Importing responses never approves them or verifies their contact addresses. Current release facts above supersede older cutover checklist states below.
...
5. With the account-name confirmation release, the server may show **Is this you?** for exactly one active, unclaimed record whose full name matches the signed-in account in that class. **Yes, this is me** connects only after the server rechecks the signed snapshot and current identity evidence. Changed, duplicate, claimed, multi-class, or conflicting-contact candidates create or reuse one officer request. **No, search again** opens the manual search. Older open pages retain the review-only policy.
...
7. A student whose verified sign-in email uniquely matches one same-class profile connects automatically. The account-name confirmation release also allows **Yes, this is me** on a unique unclaimed full-account-name match after a signed, locked recheck. Typed names without verified-email proof and conflicting evidence stay in the officer queue. Confirm the deployed release before relying on this follow-up policy.
