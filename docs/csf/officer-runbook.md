...
**Release ledger:** the current repository candidate carries 490 ordered migrations through `20260911201640_publish_dvhs_csf_1_2_29`. The private release gitlink is `98c08ab`. Signed application 1.2.29 adds verified recovery for interrupted Comments exports and record-specific discussion permissions. Production still has 482 migrations and serves usability release `84087a03`. New Sheet sync and current Home import counts await deployment and final browser checks. See the cleanup register for deployed versions and checks.
