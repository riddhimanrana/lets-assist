...
**Current status:** Active classes 2027 through 2030 each have a join code. The September 13, 2026 06:17 UTC application preview has 308 rows: 197 existing source-lineage rows and 111 rows awaiting review (109 ambiguous name-only matches and two duplicates). There are 200 persisted Fall 2026 applications. The separate 66 changed Class of 2029 historical source rows are unrelated to this application preview. A fresh historical audit found four source coordinates represented on two active profiles after earlier merges and re-imports; reconciliation remains open. Importing a response does not approve it or verify its contact addresses. See the [cleanup register](../development/cleanup-register.md) for dated counts and release evidence.
**Release ledger:** the current repository candidate carries 507 ordered migrations through `20260913061610_allow_undated_csf_activity_updates`. Its committed private release gitlink is `3fbabfb`, the signed embedded 1.2.38 source. Publication 506 preserves the reviewed 505 schema. Migration 507 applies the same optional-date rules to activity edits and draft publication. Migration 504 follows reviewed profile-merge lineage when reusing settled class-history source keys; migration 505 allows a published activity to omit all dates while retaining the date-order rules. The 1.2.38 candidate still needs release verification and copied-workbook acceptance. See the cleanup register for deployed versions, worker controls and dated checks.

**Ownership correction deployed:** Production separates reported contacts from ownership evidence and requires independently verified account connections. The two legacy connections await staff review. Workbook refresh and import processing are paused for the current release; outbound email and scheduled publishing remain disabled. See the cleanup register for exact release evidence and current reconciliation counts.
...
- The existing source-sheet decision writeback colors imported rows green for approval or red for rejection and writes one cell note. **Sync sheet** retries this queue where `CSF_SHEET_WRITEBACK_ENABLED` is enabled. This source-sheet path does not carry discussion threads.
- The new Sheet sync candidate exports separate application, point-submission, and class records. Let's Assist remains authoritative. A decision entered in Sheets becomes a proposed change that authorized staff must confirm in Let's Assist. Pending point submissions never increase verified totals.
- New destinations keep comments in Let's Assist and do not read or write Sheet comments. Existing native-thread and Comments-column destinations keep their format. For Comments-column destinations, staff review edited cells before adding them to the local history, labeled as Google Sheets edits. Comment text never approves an application or awards points.
- Use **Last synced**, **Sync now**, and **Changes to review** for routine sync work. Keep destination mappings, diagnostics, and enable/disable controls under **Advanced**. Enabled destinations poll every two minutes.
- Before enabling this candidate, mark an empty test workspace and use **Create test copy** for the authorized source workbooks. Connect only the registered copies to that workspace, with separate record IDs and no real account connections. New application and point destinations need separate empty tabs. Preserve the live source and historical class tabs.
- Test decisions, corrections, Comments edits, point resubmissions, account status, and repeat sync against the copies first. Enable real destinations only after restricted access and the copied-workbook journeys pass. A copy or comment write with an uncertain outcome needs reconciliation before retry.
- These new sync instructions describe the candidate, not completed Production acceptance. Check the cleanup register for deployed versions and verified journeys. Downloadable reports remain local formula-safe ZIP files; connecting a sync destination does not change the report download flow.
...
4. Review the committed application through **Applications → Review queue**. Approval creates or updates term membership atomically with the decision and history; neither the import nor the decision creates the profile. Account connection remains a separate ownership-verification action.
5. Keep application-reported contacts separate from account ownership. Staff can use those contacts to find the right student, but must verify identity before connecting an account.
...
| Application and login emails differ, or a reported address is shared | Use **Connect account** and record the staff identity-verification decision. Preserve both addresses. Neither a reported email nor an editable account name proves ownership. Leave uncertain matches pending. |
...
| A legacy post says scheduled                                         | Return it to drafts or publish manually. Automatic publishing is retired.                                                                                                                                      |
...
- [x] Merge and publish the private plugin first, then advance the root gitlink to private `development` `2affd09`.
...
- [ ] Verify independently established account connections, contact-only and name-only pending review, officer resolution, member search, and denial of candidate history with fictional Development records.
