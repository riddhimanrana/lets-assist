...
**Current status:** this repository carries 328 ordered migrations through `20260818223637_complete_remaining_function_acl_restoration`; Production was last verified read-only at 236 through `20260811001500`, leaving an exact repository-pinned 92-migration cutover. Hosted Development serves root merge SHA `ac99d706f95229d19265e6938df0988dd630473c` with the verified 327-migration ledger through `20260818180000`, and Google authentication redirects successfully there. The exact 328-migration hosted CI replay is the final candidate gate; final hosted browser/provider acceptance and Production email-webhook proof remain release gates. Google OAuth and Picker were previously connected for a bounded Spring 2026 application preview that committed zero applications. Production remains untouched, and schema deployment or real-data mutation requires explicit action-time approval.
...
- [ ] Deploy the ACL-complete release candidate, then prove branch-scoped non-production Supabase parity against the 328-row ordered ledger through `20260818223637`
