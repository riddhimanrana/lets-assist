...
**Release ledger:** the current repository candidate carries 493 ordered migrations through `20260911211201_csf_defer_profile_note_export`. The private release gitlink is `6b73d09`. Signed application 1.2.31 requires a complete inbound Sheet observation before staff can accept a proposed change. Production still has 482 migrations and serves usability release `84087a03`. New Sheet sync and current Home import counts await deployment and final browser checks. See the cleanup register for deployed versions and checks.
