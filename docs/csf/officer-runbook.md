...
**Current status:** Active classes 2027 through 2030 each have a join code. The latest source preview has 266 rows, with 69 identity issues awaiting staff review. There are 200 persisted Fall 2026 applications with reported course text and evidence links. A fresh historical audit found four source coordinates represented on two active profiles after earlier merges and re-imports; reconciliation remains open. Importing a response does not approve it or verify its contact addresses. See the [cleanup register](../development/cleanup-register.md) for dated counts and release evidence.
**Release ledger:** the current repository candidate carries 505 ordered migrations through `20260913031445_allow_published_csf_activity_without_start_date`. Its committed private release gitlink is `1842776`, the signed embedded 1.2.37 source. Publication 503 preserves the reviewed 502 schema; migration 504 follows reviewed profile-merge lineage when reusing settled class-history source keys; migration 505 allows a published activity to omit all dates while retaining the date-order rules. Production has 502 migrations through `20260912064503_publish_dvhs_csf_1_2_36`, host `caaafa8b`, and selected embedded CSF 1.2.36. Live Sheet sync remains disabled pending the release and copied-workbook acceptance. See the cleanup register for deployed versions and checks.
