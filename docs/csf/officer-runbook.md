...
**Release ledger:** the current repository candidate carries 485 ordered migrations through `20260911130443_publish_dvhs_csf_1_2_26`. The private release gitlink is `1af9b22`, which prepares application 1.2.27. The current ledger includes the earlier signed application 1.2.26 publication; 1.2.27 publication integration and Production activation remain pending. Production has 482 migrations and serves usability release `84087a03`. The new Sheet sync and current Home import counts remain a release candidate. Deployment receipts identify the served application separately from this database ledger.
...
- The candidate supports native cell-attached discussion threads, replies, and resolution state, including review history and author attribution. Google Sheets authors remain identified as Google Sheets authors. Comment text never approves an application or awards points. If native thread access is unavailable, keep sync blocked; a cell note is not a substitute.
...
- Test decisions, corrections, threads, point resubmissions, account status, and repeat sync against the copies first. Enable real destinations only after native thread access, restricted recipient access, and the copied-workbook journeys pass. A copy or comment write with an uncertain outcome needs reconciliation before retry.
