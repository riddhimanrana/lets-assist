...
- **Queue is not delivery.** The durable announcement ledger and `csf-communications-dispatch` route are implemented. The route authenticates before any work, accepts no organization/message input, runs only when `CSF_COMMUNICATIONS_WORKER_ENABLED` is exactly `true`, bounds each invocation to 125 attempts, and participates in the isolated cron no-egress probe. The checked-in Vercel schedule requests invocation every minute, but observed starts can be irregular. A provider-accepted status or a durable retry receipt establishes the worker outcome. **Email queued** still means only that durable queueing succeeded; delivery remains a separate provider outcome, and operator copy must not promise a fixed delivery time.
...
The no-comment transport is the default for every new destination. It leaves existing Sheet comment cells untouched and keeps the managed columns free of a Comments column. Sync does not call the native comment service, read comment cells, or write discussion history to Sheets in this mode. In-app discussions remain available in Let's Assist, and discussion-only edits do not change the Sheet source version. Native and Comments-column transports remain available for existing destinations. Copied-workbook acceptance for no-comment destinations omits only native comment receipts, Comments-cell readback, and reviewed comment edits. It still proves copied-file lineage, restricted access, provider versions, repeated exports, application decisions, point decisions, and class output before live sync can be enabled.
