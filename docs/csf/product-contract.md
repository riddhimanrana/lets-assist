...
- **Queue is not delivery.** The durable announcement ledger and `csf-communications-dispatch` route are implemented. Locally, that route authenticates before any work, accepts no organization/message input, runs only when `CSF_COMMUNICATIONS_WORKER_ENABLED` is exactly `true`, bounds each invocation, and participates in the isolated cron no-egress probe. Those controls do not by themselves establish a cadence. `vercel.json` deliberately has no CSF worker cron; `.github/workflows/csf-communications-dispatch.yml` exists on `origin/main`, the default branch, requests every ten minutes, and selects the GitHub environment named `production`. Source exposes `ENDPOINT_PATH=/api/cron/csf-communications-dispatch`; only the `CRON_BASE_URL` host is secret-held/resolved through that environment, so source cannot identify the target deployment. This proves repository schedule ownership, not current hosted behavior. The dated release record reports irregular HTTP 200 starts, but PR #153 did not re-query GitHub, the endpoint, its ledger, or its provider. That record proves neither a Development cadence, a fixed-time SLA, provider delivery, nor a Production application result. **Email queued** means only that durable queueing succeeded; no operator copy may promise automatic or fixed-time delivery until a named environment's invocation provenance and repeated route/ledger/provider outcomes are verified separately.
- **Scheduling is environment-gated publication.** The permission-rechecked, serialized, retry-safe, atomic/audited due-post publisher and its bounded auth-first route have historical migration, pgTAP, central-replay, and repository-workflow evidence. Its separate `.github/workflows/csf-scheduled-post-publisher.yml` remains absent from `origin/main`, so the workflow's presence on `development` alone does not produce scheduled runs, and it makes no fixed-time promise. A target environment must still prove exact opt-in, a successful aggregate-only invocation, and visible schedule → Feed behavior before operators rely on it there. Scheduled posts never queue email.
...
## 23. Local verification amendments — July 22 and August 6, 2026

This section began with the July 22 local verification and was extended by the
August 6 post-cleanup companion run. It supersedes earlier local test counts and
build-status statements, but it does not change the product contracts or upgrade
any external, unexecuted, or manual acceptance scope.

The 190-migration/57-table/43-file/1,279-assertion replay and 26-scenario browser
run are July 22 evidence. The 214-migration/82-table/63-file/3,165-assertion
replay, 2,337-test private suite, 40-scenario compiled browser run, and 22-image
gallery are August 6 evidence recorded by
[`20260806-post-cleanup`](evidence/20260806-post-cleanup/verification-summary.md).
Neither date is current repository, hosted Development, or Production status;
see [testing and release](testing-and-release.md) for the later
environment-separated record.

### 23.1 August 6 verified local baseline

- The August 6 namespaced isolated replay applied 214 migrations, discovered 82 CSF tables, ran 63 pgTAP files, and passed all 3,165 assertions. It used a dedicated Let’s Assist project identity, ports, containers, volumes, and network. It did not access, stop, reset, inspect, or reuse the Vela stack, and it did not target a remote Supabase project.
...
The August 6 curated visual evidence is [`evidence/20260806-post-cleanup/index.html`](evidence/20260806-post-cleanup/index.html). It contains 22 fictional screenshots plus sanitized summaries. This curated gallery is distinct from generated Playwright output, which is ignored and not retained. The compiled behavioral run passed 40 scenarios and intentionally skipped only the 3 opt-in gallery captures; live-Google acceptance remains a separate external gate. Raw traces, videos, network payloads, cookies, and storage state are not part of the curated gallery.

### 23.2 Acceptance that remained open on August 6

The following was not completed by the August 6 local run:
...
2. **Cloud development database.** No persistent Supabase `development` branch existed in the August 6 evidence. A later dated record says the branch exists, but no retained evidence proves that the required explicit `$0.01344/hour` cost authorization preceded creation; that precondition remains unverified.
...
- The durable communications ledger, the dispatch route, and the checked-in ten-minute GitHub schedule do not prove a cadence. The communications workflow exists on `origin/main` and selects the GitHub environment named `production`. Its route path `/api/cron/csf-communications-dispatch` is source-visible; only the `CRON_BASE_URL` host is secret-held, so source cannot determine which deployment was called or prove that the Production application was verified. The dated record's irregular HTTP 200 runs do not prove Development cadence or provider delivery. CLEAN-016 remains open until repeated route/ledger/provider outcomes are verified for the environment officers will rely on; queued email must not be described as delivered or guaranteed within a fixed interval.
