...
export function detectCandidateAreas(
  ...
  for (const [pageIndex, items] of itemsByPage) {
    ...
    candidates.push(
      ...detectUnderscoreCandidates(items, pageLabels, pageIndex),
    );
  }
 ...
 */
function groupByPage<T extends { pageIndex: number }>(
  items: T[],
): Map<number, T[]> {
...
function detectUnderscoreCandidates(
  ...
  for (const lineItems of lineGroups) {
    const underscoreAwareItems =
      expandLineItemsForUnderscoreDetection(lineItems);
 ...
 */
function expandLineItemsForUnderscoreDetection(
 ...
 */
function mergeBoundingBoxes(items: PdfTextItem[]): {
  x: number;
  y: number;
  width: number;
  height: number;
...
function createRightOfLabelCandidate(
  ...
  let candidateWidth = Math.min(
    baseCandidateWidthForType(label.type),
    Math.max(maxPageWidth - candidateX, 0),
  );
...
function inferTypeFromLabels(labels: DetectedLabel[]): WaiverLabelType {
  ...
  if (best.type === "phone" || /\b(phone|cell|mobile|telephone)\b/.test(text))
    return "phone";
...
function estimatePageWidth(items: PdfTextItem[]): number {
  ...
  const p95 =
    rightEdges[Math.max(0, p95Index)] ?? rightEdges[rightEdges.length - 1];
