...
test("anonymous waiver discovery is not exposed as an email oracle", () => {
  ...
  assert.doesNotMatch(actionSource, /export async function checkReusableAnonymousWaiver/u);
...
test("signed uploads are read from the private evidence bucket", () => {
  for (const route of [
    ...
    assert.doesNotMatch(source, /\.from\('waiver-uploads'\)\s*\n\s*\.download\(typedSignature\.upload_storage_path\)/u);
    assert.match(source, /\.from\('waiver-signatures'\)\s*\n\s*\.download\(typedSignature\.upload_storage_path\)/u);
  }
});
...
test("public waiver lookup resolves project visibility before service-role definition access", () => {
  ...
  const lookupEnd = source.indexOf("export async function uploadProjectWaiverPdf", lookupStart);
  ...
  assert.match(lookup, /await serviceSupabase\s*\n\s*\.from\("waiver_definitions"\)/u);
...
test("waiver definition reads and versioned saves remain scoped to their project", () => {
  ...
  const saveStart = source.indexOf("export async function saveWaiverDefinition", readStart);
  const readFlow = source.slice(readStart, saveStart);
  const saveEnd = source.indexOf("export async function", saveStart + 30);
  const saveFlow = source.slice(saveStart, saveEnd === -1 ? undefined : saveEnd);

  assert.match(readFlow, /\.eq\("id", project\.waiver_definition_id\)\s*\n\s*\.eq\("project_id", projectId\)/u);
  ...
  assert.doesNotMatch(saveFlow, /\.from\("waiver_definitions"\)\s*\n\s*\.update\(/u);
});
...
test("registered signup captures its row id before waiver persistence and plugin hooks", () => {
  ...
  const anonymousStart = source.indexOf("} else if (isAnonymous && anonymousData)", registeredStart);
  ...
  const waiverStart = source.indexOf("if ((project.waiver_required || waiverSignature) && createdSignupId)");
  const hooksStart = source.indexOf("if (project.organization_id && createdSignupId)", waiverStart);
...
test("multi-slot waiver reuse copies evidence instead of sharing cleanup paths", () => {
  const source = read("app/projects/[id]/actions.ts");
  const cloneStart = source.indexOf("async function cloneAnonymousWaiverSignatureToSignup");
  const cloneEnd = source.indexOf("export async function togglePauseSignups", cloneStart);
  ...
  assert.match(cloneFlow, /signature_storage_path: clonedSignatureStoragePath/u);
...
test("signup cancellation and project deletion cannot erase signed evidence", () => {
  ...
  assert.doesNotMatch(cancelFlow, /\.from\("project_signups"\)\s*\n\s*\.delete\(\)/u);
