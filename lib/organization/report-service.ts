...
export async function getPublicOrganizationReportSummary(
  organizationId: string,
): Promise<{ totalHours: number } | null> {
  const report = await getOrganizationReportDataForSync(organizationId);
