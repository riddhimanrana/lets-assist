...
const RETRYABLE_ERROR_CODES = new Set(["42P01", "PGRST001", "PGRST205"]);
...
function classifySupabaseQueryError(
  ...
  if (
    DETERMINISTIC_MESSAGE_SIGNALS.some((signal) =>
...
export function isRetryableSupabaseQueryError(
 ...
 */
export async function withRetryableSupabaseQuery<
  TResult extends SupabaseQueryResult<unknown>,
>(
  query: () => PromiseLike<TResult> | TResult,
  options: RetryOptions = {},
): Promise<TResult> {
  const { maxAttempts = 3, initialDelayMs = 250, maxDelayMs = 1500 } = options;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (
      !lastResult.error ||
      !isRetryableSupabaseQueryError(lastResult.error as SupabaseQueryErrorLike)
    ) {
      return lastResult;
    }
