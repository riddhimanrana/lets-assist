...
export function FieldListPanel({
  ...
  const mappedSignaturesCount = signatureFields.filter(
    (f) => fieldMappings[f.fieldName]?.signerRoleKey,
  ).length;
  const unassignedSignaturesCount =
    signatureFields.length - mappedSignaturesCount;
  const requiredFieldsCount = detectedFields.filter(
    (f) => fieldMappings[f.fieldName]?.required ?? f.required,
  ).length;
  ...
  const renderFieldItem = (field: DetectedPdfField) => {
    ...
    const isMapped =
      field.fieldType === "signature" ? !!mapping.signerRoleKey : true;
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span
              className="font-medium text-sm truncate max-w-36"
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  updateMapping(field, {
                    signerRoleKey:
                      val === "unassigned" || !val ? undefined : val,
                  })
                }
              >
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                updateMapping(field, { required: checked })
              }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {unassignedSignaturesCount > 0 && (
        <div className="shrink-0 mb-3 mx-1 p-2 bg-warning/10 border border-warning/40 rounded text-xs text-warning flex items-start gap-2">
          ...
          <span>
            There are {unassignedSignaturesCount} signature fields not assigned
            to any role. Signers might not see where to sign.
          </span>
        </div>
      )}
      ...
      <ScrollArea className="flex-1 pr-4">
        <Accordion
          defaultValue={["signature-fields", "other-fields"]}
          className="w-full"
        >
            ...
            <AccordionContent className="pt-2">
              {signatureFields.length === 0 ? (
                <div className="text-sm text-muted-foreground p-4 text-center">
                  No signature fields detected. Use the custom placements
                  section below to add signature boxes.
                </div>
              ) : (
