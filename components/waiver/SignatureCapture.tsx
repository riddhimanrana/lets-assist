...
export function SignatureCapture({
  ...
  const [method, setMethod] = useState<"draw" | "typed" | "upload">(
    initialMethod,
  );
  const [drawn, setDrawn] = useState(false);
  const [typedValue, setTypedValue] = useState(
    existingSignature?.method === "typed"
      ? existingSignature.data
      : userName || "",
  );
  const [uploadDataUrl, setUploadDataUrl] = useState<string | null>(
    existingSignature?.method === "upload" ? existingSignature.data : null,
  );
  ...
  useEffect(() => {
    if (method === "typed") {
      emitSignature(
        typedValue.trim().length > 1 ? typedValue.trim() : null,
        "typed",
      );
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Label>
          Signature for{" "}
          <span className="font-semibold text-primary">{signerRole.label}</span>
        </Label>
        {signerRole.required && (
      ...
      >
        <TabsList
          className={`grid w-full ${allowUpload ? "grid-cols-3" : "grid-cols-2"}`}
        >
          <TabsTrigger
            value="draw"
          ...
          <TabsTrigger
            value="typed"
            className="flex gap-2"
            data-testid="signature-tab-typed"
          >
          ...
          {allowUpload && (
            <TabsTrigger
              value="upload"
              className="flex gap-2"
              data-testid="signature-tab-upload"
            >
