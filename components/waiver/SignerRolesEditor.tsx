...
export function SignerRolesEditor({
  ...
  const handleUpdateSigner = (
    index: number,
    updates: Partial<WaiverDefinitionSignerInput>,
  ) => {
  ...
  const handleMoveSigner = (index: number, direction: "up" | "down") => {
    ...
    // Swap
    [newSigners[index], newSigners[targetIndex]] = [
      newSigners[targetIndex],
      newSigners[index],
    ];
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        handleUpdateSigner(index, { required: checked })
                      }
                      disabled={readOnly}
              ...
              <Tooltip>
                <TooltipTrigger>
                  <span
                    ...
                    data-disabled={
                      signers.length <= 1 || readOnly ? "true" : undefined
                    }
                    style={{
                      pointerEvents:
                        signers.length <= 1 || readOnly ? "none" : "auto",
                      opacity: signers.length <= 1 || readOnly ? 0.5 : 1,
                    }}
                  >
