...
function LocationAutocompleteContent({
          ...
          setPredictions(
            suggestions
              ...
              .filter(
                (
                  placePrediction,
                ): placePrediction is google.maps.places.PlacePrediction =>
  ...
  const handleSelect = async (prediction: LocationPrediction) => {
    ...
    try {
      ...
      const locationData: LocationData = {
        ...
        display_name:
          place.formattedAddress || prediction.placePrediction.text.text,
        coordinates: {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newValue !== value?.text) {
      onChangeAction(
        newValue ? { text: newValue, display_name: newValue } : undefined,
      );
    }
  };
    ...
    <div className={cn("relative space-y-1.5", className)} ref={containerRef}>
      <div className="relative">
        <Input
          ...
          className={cn("w-full pl-9", error && "border-destructive")}
          maxLength={maxLength}
          required={required}
                ...
                {query.length < 3 && query.length > 0 ? (
                  ...
                  <div className="py-6 text-center flex flex-row justify-center items-center gap-4 px-4">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Please wait while we find matching locations...
                      </p>
                    </div>
                  </div>
                ) : (
              ...
              <CommandGroup>
                {predictions.map((prediction, index) => {
                  const isCurrentSelection =
                    prediction.kind === "current-selection";
      ...
      {error && errorMessage && (
        <div
          id={ariaErrorMessage}
          className="text-destructive text-sm flex items-center gap-1.5 mt-1"
        >
