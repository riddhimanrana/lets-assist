 ...
 */
export function shouldShowTimezoneBadge(
...
export function getBestMatchingTimezone(detectedTimezone: string): string {
  ...
  if (detectedTimezone.startsWith("Asia/")) {
    ...
    if (
      detectedTimezone.includes("Kolkata") ||
      detectedTimezone.includes("India")
    ) {
      return "Asia/Kolkata";
    }
    // Check for other major Asian timezones
    if (
      detectedTimezone.includes("Shanghai") ||
      detectedTimezone.includes("Hong_Kong") ||
      detectedTimezone.includes("Singapore")
    ) {
      return "Asia/Shanghai";
    }
    if (
      detectedTimezone.includes("Tokyo") ||
      detectedTimezone.includes("Seoul")
    ) {
      return "Asia/Tokyo";
    }
