...
V85: CSF account claim → exact unique verified school or personal email only; response email, preferred-contact email, and account email remain distinct evidence; names never auto-link or create profiles; ambiguous/conflicting matches enter officer review.
...
V89: CSF email → shared sender interface supports plain text, reply-to, tags, provider idempotency key, and sender override while local development remains Mailpit; campaign audience snapshots, deliveries, and provider events are durable; raw-body webhook signatures and provider event IDs are verified/deduplicated before state changes.
V90: CSF navigation → one Let’s Assist organization workspace header; desktop keeps Home, Applications, Members, Service, Semester, and More; phone exposes every authorized destination in one full section switcher; Share is a page action, never a competing navigation row.
...
V92: CSF provider mutation → visibly verified chapter account + purpose-bound minimum scope + exact console/config diff; Gmail remains reply/evidence only and never a send transport; Resend topic/webhook creation and Google console changes occur only after local delivery/sync acceptance.
...
T38|.|extend source snapshots, atomic generic import commit, term-scoped club representatives, append-only club lifecycle, calendar bindings, and durable CSF communications without duplicating the existing domain|V24,V27,V33,V40,V49,V84,V85,V86,V87,V88,V89,V93,I.schema,I.service
