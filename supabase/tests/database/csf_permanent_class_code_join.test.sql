...
SELECT extensions.is((SELECT payload->>'needsReview' FROM class_join_replay_results WHERE scenario='new'),'true','a new student reaches staff review instead of creating a profile');
SELECT extensions.is((SELECT payload->>'profileId' FROM class_join_replay_results WHERE scenario='new'),NULL,'an unmatched join names no record');
SELECT extensions.is((SELECT count(*) FROM plugin_data.csf_profiles WHERE organization_id='ca200000-0000-4000-8000-000000000001' AND source_summary->>'createdBy'='permanent_class_code'),0::bigint,'a class code never creates a profile');
