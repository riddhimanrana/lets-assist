...
UPDATE plugin_data.csf_profiles SET personal_email=NULL,normalized_personal_email=NULL WHERE last_name='Newapplicant';
...
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Newapplicant'),'new-applicant@local.test','application commit fills the existing resolved profile blank contact');
...
CREATE TEMP TABLE captured_contacts AS SELECT count(*) AS total FROM plugin_data.csf_admin_audit_events WHERE action='profile.application_contacts_captured';
UPDATE plugin_data.csf_profiles SET personal_email='officer-updated@local.test',normalized_personal_email='officer-updated@local.test' WHERE last_name='Newapplicant';
...
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Newapplicant'),'officer-updated@local.test','a replay preserves the later officer contact correction');
SELECT extensions.is((SELECT count(*) FROM plugin_data.csf_admin_audit_events WHERE action='profile.application_contacts_captured'),(SELECT total FROM captured_contacts),'commit replay adds no duplicate contact audit');
