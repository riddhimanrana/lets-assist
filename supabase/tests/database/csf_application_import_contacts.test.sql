...
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Fixture5'),'agree@local.test','agreeing contacts normalize into one personal address');
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Fixture6'),'preferred@local.test','preferred contact takes precedence over a distinct response address');
SELECT extensions.is((SELECT normalized_data#>>'{record,contact,responseEmail}' FROM plugin_data.csf_sheet_import_rows WHERE row_number=6),'response@local.test','distinct response remains immutable source evidence');
SELECT extensions.ok((SELECT school_email IS NULL FROM plugin_data.csf_profiles WHERE last_name='Fixture6'),'response email is not mislabeled as school email');
SELECT extensions.ok((SELECT personal_email IS NULL AND school_email IS NULL FROM plugin_data.csf_profiles WHERE last_name='Fixture7'),'blank addresses stay absent');
SELECT extensions.ok((SELECT personal_email IS NULL AND school_email IS NULL FROM plugin_data.csf_profiles WHERE last_name='Fixture8'),'malformed addresses stay absent');
SELECT extensions.is((SELECT school_email FROM plugin_data.csf_profiles WHERE last_name='Fixture9'),'school@students.local.test','explicit school contact is retained');
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Fixture9'),'home@local.test','explicit school and preferred contacts remain separate');
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Fixture10'),'fallback@local.test','declared invalid preferred evidence falls back to valid response');
...
CREATE TEMP TABLE contact_audit_before AS SELECT count(*) AS total FROM plugin_data.csf_admin_audit_events WHERE action='profile.application_contacts_captured';
...
SELECT extensions.is((SELECT count(*) FROM plugin_data.csf_admin_audit_events WHERE action='profile.application_contacts_captured'),(SELECT total FROM contact_audit_before),'replay adds no contact audit');
UPDATE plugin_data.csf_profiles SET personal_email='staff-corrected@local.test',normalized_personal_email='staff-corrected@local.test' WHERE last_name='Fixture6';
SELECT plugin_data.csf_fill_application_profile_contacts('cfc10000-0000-4000-8000-000000000001',id,matched_profile_id,'cfc00000-0000-4000-8000-000000000001') FROM plugin_data.csf_sheet_import_rows WHERE row_number=6;
SELECT extensions.is((SELECT personal_email FROM plugin_data.csf_profiles WHERE last_name='Fixture6'),'staff-corrected@local.test','existing officer contact survives later capture');
UPDATE plugin_data.csf_profiles SET personal_email=NULL,normalized_personal_email=NULL WHERE last_name='Fixture6';
UPDATE plugin_data.csf_profiles SET personal_email='preferred@local.test',normalized_personal_email='preferred@local.test' WHERE id='cfc40000-0000-4000-8000-000000000001';
SELECT plugin_data.csf_fill_application_profile_contacts('cfc10000-0000-4000-8000-000000000001',id,matched_profile_id,'cfc00000-0000-4000-8000-000000000001') FROM plugin_data.csf_sheet_import_rows WHERE row_number=6;
SELECT extensions.ok((SELECT personal_email IS NULL FROM plugin_data.csf_profiles WHERE last_name='Fixture6'),'contact owned by another profile is skipped');
SELECT extensions.ok(EXISTS(SELECT 1 FROM plugin_data.csf_admin_audit_events WHERE action='profile.application_contacts_captured' AND after_data->'skippedConflictingFields' ? 'personalEmail'),'contact collision is audited');
