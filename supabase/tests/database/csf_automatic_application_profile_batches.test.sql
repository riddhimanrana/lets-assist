...
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_profiles WHERE organization_id='cfc10000-0000-4000-8000-000000000001' AND (reported_application_school_email IS NOT NULL OR reported_application_personal_email IS NOT NULL) AND school_email IS NULL AND personal_email IS NULL AND normalized_school_email IS NULL AND normalized_personal_email IS NULL),51,'all new applicants retain reported contacts without changing identity fields');
