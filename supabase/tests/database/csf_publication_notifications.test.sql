...
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),0,'installing the migration does not backfill publication events');
...
INSERT INTO publication_test_ids SELECT 'post',(plugin_data.csf_mutate_post('ec100000-0000-4000-8000-000000000001','create',NULL,'{"title":"Fictional post","body":"Synthetic text.","audience":"members","publish":true,"sendEmail":false}', 'ec200000-0000-4000-8000-000000000001','ec600000-0000-4000-8000-000000000001')->>'postId')::uuid;
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),1,'create-published records one event in the publication transaction');
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_notification_deliveries),5,'organization-wide post recipients follow the current member-feed visibility');
SELECT extensions.lives_ok($q$SELECT plugin_data.csf_mutate_post('ec100000-0000-4000-8000-000000000001','create',NULL,'{"title":"Fictional post","body":"Synthetic text.","audience":"members","publish":true,"sendEmail":false}','ec200000-0000-4000-8000-000000000001','ec600000-0000-4000-8000-000000000001')$q$,'same request can replay');
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),1,'publication retry does not duplicate its event');
...
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),1,'ordinary edits do not notify');
...
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_notification_deliveries d JOIN plugin_data.csf_publication_events e ON e.id=d.event_id WHERE e.source_kind='activity'),5,'chapter-wide activities follow the public activity reader for active organization members');
...
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),2,'draft creation records no publication');
...
SELECT plugin_data.csf_set_activity_status('ec100000-0000-4000-8000-000000000001',(SELECT id FROM publication_test_ids WHERE kind='draft'),'published','Fictional publication','ec200000-0000-4000-8000-000000000001','ec600000-0000-4000-8000-000000000004');
SELECT extensions.is((SELECT count(*)::integer FROM plugin_data.csf_publication_events),3,'draft publication and replay produce one event');
INSERT INTO plugin_data.csf_announcements(id,organization_id,title,body,audience,audience_cohort_id,status,created_by,updated_by) VALUES
('ec700000-0000-4000-8000-000000000001','ec100000-0000-4000-8000-000000000001','Fictional class post','Synthetic text.','class','ec400000-0000-4000-8000-000000000001','published','ec200000-0000-4000-8000-000000000001','ec200000-0000-4000-8000-000000000001'),
('ec700000-0000-4000-8000-000000000002','ec100000-0000-4000-8000-000000000001','Fictional officer post','Synthetic text.','officers',NULL,'published','ec200000-0000-4000-8000-000000000001','ec200000-0000-4000-8000-000000000001');
...
-- Keep the actual PostgreSQL definitions and ACLs in the isolated CI test log.
SELECT extensions.diag(receipt::text) FROM (
