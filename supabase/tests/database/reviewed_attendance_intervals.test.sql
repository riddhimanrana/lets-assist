...
SELECT extensions.is(private.attendance_interval_minutes(private.normalize_attendance_intervals('[{"checkIn":"2026-09-18T23:30:00Z","checkOut":"2026-09-19T01:00:00Z"}]')),90,'explicit overnight dates preserve the actual duration');
SELECT extensions.throws_ok($$SELECT private.normalize_attendance_intervals('[{"checkIn":"2026-09-18T23:30:00Z","checkOut":"2026-09-18T01:00:00Z"}]')$$,'22023','attendance intervals must be positive and disjoint','reversed times never infer an overnight date');
SELECT extensions.is(private.attendance_interval_minutes(private.normalize_attendance_intervals('[{"checkIn":"2026-09-18T09:00:00Z","checkOut":"2026-09-19T09:00:00Z"}]')),1440,'the existing 24 hour boundary remains valid');
...
CREATE TEMP TABLE fresh_excluded_row AS SELECT public.add_paper_attendance_row('a7100000-0000-4000-8000-000000000001',(SELECT id FROM manual),'a7000000-0000-4000-8000-000000000001','a7500000-0000-4000-8000-000000000099') AS id;
SELECT extensions.is(public.update_paper_scan_review_row((SELECT id FROM manual),'a7100000-0000-4000-8000-000000000001',(SELECT id FROM fresh_excluded_row),'a7000000-0000-4000-8000-000000000001','{"expectedRevision":0,"decision":"exclude"}'),'updated','fresh unsaved rows can still be excluded');
SELECT extensions.is((SELECT decision FROM public.project_paper_scan_rows WHERE id=(SELECT id FROM fresh_excluded_row)),'exclude','unsaved exclusion persists normally');
...
CREATE TEMP TABLE saved_before_exclusion AS SELECT to_jsonb(rows) AS row_state,to_jsonb(batches) AS batch_state,to_jsonb(roster) AS roster_state
FROM public.project_paper_scan_rows rows JOIN public.project_paper_scan_batches batches ON batches.id=rows.batch_id JOIN public.project_paper_roster_entries roster ON roster.scan_row_id=rows.id WHERE rows.id=(SELECT id FROM manual_row);
SELECT extensions.throws_ok($$SELECT public.update_paper_scan_review_row((SELECT id FROM manual),'a7100000-0000-4000-8000-000000000001',(SELECT id FROM manual_row),'a7000000-0000-4000-8000-000000000001','{"expectedRevision":1,"decision":"exclude"}')$$,'22023','saved attendance must remain included','terminal saved roster rejects exclusion');
SELECT extensions.ok((SELECT to_jsonb(rows)=prior.row_state AND to_jsonb(batches)=prior.batch_state AND to_jsonb(roster)=prior.roster_state FROM public.project_paper_scan_rows rows JOIN public.project_paper_scan_batches batches ON batches.id=rows.batch_id JOIN public.project_paper_roster_entries roster ON roster.scan_row_id=rows.id CROSS JOIN saved_before_exclusion prior WHERE rows.id=(SELECT id FROM manual_row)),'terminal refusal rolls back batch reopening, row revision, and roster mutations');
...
CREATE TEMP TABLE reopened_before_exclusion AS SELECT to_jsonb(rows) AS row_state,to_jsonb(batches) AS batch_state,to_jsonb(roster) AS roster_state
FROM public.project_paper_scan_rows rows JOIN public.project_paper_scan_batches batches ON batches.id=rows.batch_id JOIN public.project_paper_roster_entries roster ON roster.scan_row_id=rows.id WHERE rows.id=(SELECT id FROM manual_row);
...
SELECT extensions.throws_ok($$SELECT public.update_paper_scan_review_row((SELECT id FROM manual),'a7100000-0000-4000-8000-000000000001',(SELECT id FROM manual_row),'a7000000-0000-4000-8000-000000000001','{"expectedRevision":2,"name":"New transcription"}')$$,'22023','saved attendance must remain included','omitted decision cannot retain an inconsistent saved exclusion');
SELECT extensions.is(public.update_paper_scan_review_row((SELECT id FROM manual),'a7100000-0000-4000-8000-000000000001',(SELECT id FROM manual_row),'a7000000-0000-4000-8000-000000000001','{"expectedRevision":2,"decision":"include"}'),'updated','explicit inclusion restores an inconsistent saved row for identity resolution');
...
SELECT extensions.is(public.update_paper_scan_review_row((SELECT id FROM manual),'a7100000-0000-4000-8000-000000000001',(SELECT id FROM manual_row),'a7000000-0000-4000-8000-000000000001','{"expectedRevision":3,"decision":"exclude"}'),'already_committed','promoted signup retains its existing immutable-review response');
SELECT extensions.ok((SELECT decision='include' AND review_revision=3 AND committed_signup_id IS NOT NULL FROM public.project_paper_scan_rows WHERE id=(SELECT id FROM manual_row)),'committed signup exclusion changes no review state');
