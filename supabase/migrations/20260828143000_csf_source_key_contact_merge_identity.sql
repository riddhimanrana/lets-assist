-- Let an exact class-workbook roster key corroborate imported profile identity
-- even when one record already carries contact data. The key must be derived
-- from the record's normalized name, belong to the same official workbook and
-- class, and point to a different semester tab or the same exact coordinate.
-- All relationship, semester, account, cohort, and differing-email conflicts
-- remain authoritative.

ALTER FUNCTION plugin_data.csf_profile_merge_preview(uuid, uuid, uuid)
  RENAME TO csf_profile_merge_preview_source_key_contact_base;
...
BEGIN
  v_preview := plugin_data.csf_profile_merge_preview_source_key_contact_base(
    p_organization_id,
    p_source_profile_id,
    p_target_profile_id
  );
  ...
  INTO v_exact_source_key_identity
  ...
  CROSS JOIN LATERAL (
    SELECT
      pg_catalog.lower(pg_catalog.regexp_replace(
        source_row.normalized_data #>> '{identity,sourceStudentKey}',
        '[[:space:]]+', '', 'g'
      )) AS source_key,
      pg_catalog.lower(pg_catalog.regexp_replace(
        target_row.normalized_data #>> '{identity,sourceStudentKey}',
        '[[:space:]]+', '', 'g'
      )) AS target_key,
      pg_catalog.lower(pg_catalog.regexp_replace(
  ...
  v_preview := pg_catalog.jsonb_set(
    v_preview,
    '{identityEvidence,exactSourceStudentKeyMatch}',
    pg_catalog.to_jsonb(v_exact_source_key_identity),
    true
  );
...
REVOKE ALL ON FUNCTION plugin_data.csf_profile_merge_preview_source_key_contact_base(uuid, uuid, uuid)
...
-- Invalidate any cached PL/pgSQL dependency plan that resolved the canonical
-- preview before it was renamed above. The locked merge must re-resolve the
-- new canonical preview instead of calling the prior base function by OID.
...
COMMENT ON FUNCTION plugin_data.csf_profile_merge_preview_source_key_contact_base(uuid, uuid, uuid) IS
  'Internal merge preview before exact class-workbook roster-key identity corroboration. Not client executable.';
COMMENT ON FUNCTION plugin_data.csf_profile_merge_preview(uuid, uuid, uuid) IS
  'Returns the canonical CSF profile merge preview. An exact name-derived roster key from one official class workbook may replace only the missing-email blocker; every concrete identity and relationship conflict remains authoritative.';
