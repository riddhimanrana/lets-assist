...
BEGIN
  ...
  IF v_row.resolution_status IS DISTINCT FROM 'resolved' THEN
    IF v_row.resolution_status = 'pending'
      AND p_batch_id IS NOT NULL
      AND v_row.resolution_metadata ->> 'revertedBatchId' = p_batch_id::text
      AND v_row.resolution_metadata ->> 'batchId' = p_batch_id::text
    ...
    END IF;
    -- A different batch or untouched row has no successful receipt to replay.
    RETURN jsonb_build_object(
      'rowId', v_row.id, 'reverted', false, 'reason', 'not_resolved'
    );
  END IF;

  -- Recheck under the identity and row locks. A later resolution owns its batch.
  IF p_batch_id IS NULL
    OR v_row.resolution_metadata ->> 'batchId' IS DISTINCT FROM p_batch_id::text THEN
    RAISE EXCEPTION 'This row no longer belongs to the requested confirmation batch.'
      USING ERRCODE = 'check_violation';
  END IF;
