-- Keep pending applications in the application review workflow.
BEGIN;
...
BEGIN
  ...
  -- An application owns the semester outcome even before it has a membership.
  -- The outer wrapper already holds the organization and profile identity locks.
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION
      'This semester already has an application. Use application review instead of importing class history.'
      USING ERRCODE = '23514';
  END IF;
...
CREATE OR REPLACE FUNCTION plugin_data.csf_class_import_review_rows(
  p_organization_id uuid,
  p_job_id uuid,
