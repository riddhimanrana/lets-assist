-- Queue an audited officer re-parse without changing immutable import evidence.

CREATE UNIQUE INDEX csf_workbook_reprepare_request_idx
  ON plugin_data.csf_admin_audit_events (organization_id, correlation_id)
  WHERE action = 'sheets.class_workbook_reprepare_requested';

CREATE FUNCTION plugin_data.csf_request_class_workbook_reprepare(
