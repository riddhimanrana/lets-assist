-- Contact emails and editable names are suggestions, not ownership proof.
BEGIN;
...
CREATE OR REPLACE FUNCTION plugin_data.csf_confirm_class_code_account_name_match(
  ...
  p_normalized_first_name text, p_normalized_last_name text, p_account_name_hash text
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
DECLARE v_code text;
BEGIN
  PERFORM plugin_data.csf_lock_identity_mutation(p_organization_id);
  SELECT code.code INTO v_code FROM plugin_data.csf_class_join_codes code
    WHERE code.organization_id = p_organization_id AND code.id = p_class_join_code_id
      AND code.cohort_id = p_cohort_id AND code.status = 'active' FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'This CSF class code is no longer active.'; END IF;
  RETURN plugin_data.csf_join_class_by_code(p_organization_id, v_code, p_user_id,
    p_verified_email, p_normalized_first_name, p_normalized_last_name, NULL,
    coalesce(p_profile_id, '00000000-0000-0000-0000-000000000000'::uuid), NULL);
END;
$$;
REVOKE ALL ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match(uuid,uuid,uuid,text,uuid,uuid,text,text,text)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match(uuid,uuid,uuid,text,uuid,uuid,text,text,text) TO service_role;

CREATE OR REPLACE FUNCTION plugin_data.csf_confirm_class_code_account_name_match_v4(
  p_organization_id uuid, p_profile_id uuid, p_user_id uuid,
  p_verified_email text, p_class_join_code_id uuid, p_cohort_id uuid,
  p_normalized_first_name text, p_normalized_last_name text, p_account_name_hash text
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
...
REVOKE ALL ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match_v4(uuid,uuid,uuid,text,uuid,uuid,text,text,text)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match_v4(uuid,uuid,uuid,text,uuid,uuid,text,text,text) TO service_role;

CREATE OR REPLACE FUNCTION plugin_data.csf_confirm_class_code_account_name_match_identity_base(
  p_organization_id uuid, p_profile_id uuid, p_user_id uuid,
  p_verified_email text, p_class_join_code_id uuid, p_cohort_id uuid,
  p_normalized_first_name text, p_normalized_last_name text, p_account_name_hash text
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = '' AS $$
...
END;
$$;
REVOKE ALL ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match_identity_base(uuid,uuid,uuid,text,uuid,uuid,text,text,text)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_confirm_class_code_account_name_match_identity_base(uuid,uuid,uuid,text,uuid,uuid,text,text,text) TO postgres;


-- Staff search reads confirmed login addresses; reported contacts never establish ownership.
CREATE OR REPLACE FUNCTION plugin_data.csf_search_organization_accounts(
  p_organization_id uuid, p_actor_user_id uuid, p_query text
...
BEGIN
  IF NOT plugin_data.csf_actor_has_permission(p_organization_id,p_actor_user_id,'manage_profiles') THEN
    RAISE EXCEPTION 'Not authorized to search CSF organization accounts.' USING ERRCODE='42501';
  END IF;
  IF v_query IS NULL OR length(v_query)<2 OR length(v_query)>100 THEN
    RAISE EXCEPTION 'Enter a name or login email of 2 to 100 characters.' USING ERRCODE='22023';
  END IF;
  RETURN coalesce((SELECT jsonb_agg(jsonb_build_object('id',matched.id,'name',matched.name,'email',matched.email)
    ORDER BY matched.name,matched.id) FROM (
      SELECT u.id,coalesce(nullif(btrim(p.full_name),''),nullif(btrim(p.username),''),u.email) AS name,u.email
      ...
      WHERE m.organization_id=p_organization_id AND m.status='active'
        ...
        AND (position(v_query in lower(coalesce(p.full_name,'')))>0
          OR position(v_query in lower(coalesce(p.username,'')))>0
          OR position(v_query in lower(u.email))>0)
      ORDER BY name,u.id LIMIT 10
    ) matched), '[]'::jsonb);
END;
$$;
REVOKE ALL ON FUNCTION plugin_data.csf_search_organization_accounts(uuid,uuid,text)
  FROM PUBLIC,anon,authenticated,service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_search_organization_accounts(uuid,uuid,text) TO service_role;

-- Record each request resolved by the staff identity decision.
