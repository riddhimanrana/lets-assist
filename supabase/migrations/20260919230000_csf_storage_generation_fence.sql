...
-- A Storage delete can begin before its database claim expires and finish
-- after a later worker settles that claim. Bind each post-image object path to
-- the immutable publication request so a stale worker can only remove the
-- retired generation it originally claimed.
ALTER TABLE plugin_data.csf_announcement_attachments
  ...
  ADD CONSTRAINT csf_announcement_attachments_object_path_scope_check CHECK (
    object_path = organization_id::text || '/dvhs-csf/post-images/'
      ...
      END
    OR (
      restore_request_id IS NOT NULL
      AND object_path = organization_id::text || '/dvhs-csf/post-images/'
        || announcement_id::text || '/' || restore_request_id::text || '/'
        || checksum_sha256
        || CASE mime_type
          WHEN 'image/jpeg' THEN '.jpg'
          WHEN 'image/png' THEN '.png'
    ...
    ELSE
      ...
      IF v_mime_type NOT IN ('image/jpeg', 'image/png', 'image/webp')
        ...
        OR v_object_path IS DISTINCT FROM (p_organization_id::text
          ...
          || p_request_id::text || '/' || v_checksum || CASE v_mime_type
            WHEN 'image/jpeg' THEN '.jpg'
            WHEN 'image/png' THEN '.png'
            WHEN 'image/webp' THEN '.webp'
          END) THEN
      ...
      IF EXISTS (
        ...
        WHERE attachment.organization_id = p_organization_id
          AND attachment.announcement_id = p_announcement_id
          AND attachment.checksum_sha256 = v_checksum
      ) THEN
        RAISE EXCEPTION 'That post image already exists. Keep the existing image instead.'
          USING ERRCODE = '22023';
      END IF;
...
-- The insert trigger must consume a preparation for the new generated path.
-- Legacy rows remain readable, but no new restore may publish a legacy path.
...
BEGIN
  ...
  v_expected_path := NEW.organization_id::text || '/dvhs-csf/post-images/'
    || NEW.announcement_id::text || '/' || NEW.restore_request_id::text || '/'
    || NEW.checksum_sha256
    || CASE NEW.mime_type
      WHEN 'image/jpeg' THEN '.jpg'
      WHEN 'image/png' THEN '.png'
      ...
      ELSE ''
    END;
  v_legacy_path := NEW.organization_id::text || '/dvhs-csf/post-images/'
    ...
    || CASE NEW.mime_type
      ...
      ELSE ''
    END;
  IF NEW.object_path IS DISTINCT FROM v_expected_path
    AND NEW.object_path IS DISTINCT FROM v_legacy_path THEN
    RETURN NEW;
  END IF;
  ...
  IF FOUND AND v_queue.claim_token IS NOT NULL THEN
    RAISE EXCEPTION 'Post attachment restoration must retry after storage cleanup reconciliation.'
      USING ERRCODE = '40001';
  END IF;
  IF NOT FOUND OR v_queue.organization_id IS DISTINCT FROM NEW.organization_id THEN
...
-- Organization teardown is already serialized by the organization advisory
-- lock. Cancel unconsumed upload preparations before counting the preserved
-- queue so local seed reset and uninstall can claim the queued paths now rather
-- than waiting for a 15-minute lease that no live writer may finish.
...
DECLARE
  ...
  v_remaining_preparations integer := 0;
...
BEGIN
  ...
  DELETE FROM plugin_data.csf_attachment_restore_preparations
  WHERE organization_id = p_organization_id;
  GET DIAGNOSTICS v_remaining_preparations = ROW_COUNT;
  v_preparations := v_preparations + v_remaining_preparations;
...
COMMENT ON FUNCTION plugin_data.csf_replace_post_attachments(
  ...
  'Service-only, permission-revalidated replacement of a post image set. New uploads use the publication request as a Storage generation so an expired cleanup worker cannot delete a later restore.';
COMMENT ON FUNCTION plugin_data.csf_reconcile_attachment_restore_cleanup() IS
  'Consumes the active request-generation restore lease and cancels its cleanup row atomically with attachment metadata insertion.';
COMMENT ON FUNCTION plugin_data.csf_purge_storage_deletion_queue(uuid) IS
  'Cancels unfinished restore preparations, then runs two-phase organization Storage teardown without erasing unresolved cleanup work.';
