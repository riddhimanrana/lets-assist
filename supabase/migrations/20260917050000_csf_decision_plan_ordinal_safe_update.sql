-- 20260917040000 cleared the three plan tables with TRUNCATE so the reset
-- would survive the request role's safe-update guard. It left one write that
-- the same guard rejects: csf_stage_sheet_application_decisions computes
-- will_apply and will_retract with an UPDATE of pg_temp.csf_decision_row_plan
-- that has no WHERE clause, so the RPC still fails with "UPDATE requires a
-- WHERE clause" when the product calls it. pgTAP and direct psql do not run
-- under that session policy and so do not see it.
--
-- Every row of the plan is inserted with a WITH ORDINALITY ordinal, and
-- ordinal is the join key the preceding UPDATE already uses. WHERE ordinal IS
-- NOT NULL therefore matches the whole table: it satisfies the guard and
-- changes no row's verdict.
--
-- Nothing else moves. csf_stage_sheet_application_decisions is restated
-- verbatim from 20260917040000 with only that one predicate added, and its
-- REVOKE/GRANT restated unchanged. csf_release_sheet_application_decisions is
-- not touched; both of its writes already carry a WHERE clause. No guard is
-- weakened and no session setting is altered.
