-- Prevent a scoped application import from replaying an unrelated ordinary
-- approval batch that happens to use the same caller request ID.
BEGIN;
...
BEGIN
  ...
  -- Match the ordinary batch path: authorize under the staff lock, then take
  -- the request coordinate before any preview or identity mutation lock.
  PERFORM pg_catalog.pg_advisory_xact_lock(
  ...
  -- An existing scoped receipt is handled below as an idempotent replay. A
  -- request already owned by the general approval ledger must be rejected
  -- before the parent row or any derived record changes.
  IF EXISTS (
    SELECT 1
...
COMMENT ON FUNCTION plugin_data.csf_queue_scoped_application_import(
  uuid, uuid, uuid, uuid, timestamptz, uuid, text
) IS 'Derives and queues exactly one officer-resolved application row from a sealed source preview. It reserves the caller request ID before changing the parent and refuses IDs already bound to another approval batch.';
