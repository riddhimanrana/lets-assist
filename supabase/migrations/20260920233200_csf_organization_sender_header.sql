-- Quote organization display names in new notification sender headers.
BEGIN;
    ...
    'providerPayload', pg_catalog.jsonb_strip_nulls(
      pg_catalog.jsonb_build_object(
        'from', pg_catalog.concat(
          CASE WHEN v_campaign.sender_email = 'updates@notifications.lets-assist.com'
            THEN to_jsonb(coalesce(v_campaign.sender_name, ''))::text
            ELSE coalesce(v_campaign.sender_name, '') END, ' <',
