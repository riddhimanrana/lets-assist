-- Keep committed application contacts visible to officers as discovery evidence,
-- but never treat a reported application address as proof that an account owns
-- a student record. Only one unique staff-curated school or personal email may
-- authorize the connection after every existing identity and conflict check.
...
BEGIN
  ...
  END IF;
  -- Application contacts are officer-visible discovery evidence only. They may
  -- belong to a guardian, be shared, or be mistyped, so they never authorize
  -- access to a student's private record.
  ...
  THEN
    v_blockers := array_append(
      v_blockers,
      CASE
        WHEN v_application_source_email_overlap THEN
          'The application contact matches this account, but application contacts cannot verify student-record ownership. Confirm a school or personal email on the student record before connecting.'
        ELSE
          'The account''s confirmed email does not match this student record.'
      END
    );
  ELSIF v_exact_email_overlap
    AND v_verified_email_profile_matches <> 1
  THEN
    v_blockers := array_append(
      v_blockers,
...
COMMENT ON FUNCTION plugin_data.csf_profile_link_connect_evidence(uuid,uuid,uuid) IS
  'Officer-only connection evidence. Requires a current confirmed account email that uniquely matches a staff-curated school or personal email on the active student record, plus exact name, one active class, and an unlinked target. Committed application contacts remain advisory discovery evidence and never authorize access.';
