-- Parse retry ranges with the same bounded A1 grammar as import readiness.
    ...
    FOR v_depth IN 1..32 LOOP
      ...
      IF v_origin_job.mapping_version IS DISTINCT FROM v_job.mapping_version THEN
        ...
        FOR v_index IN 1..2 LOOP
          v_range := CASE WHEN v_index=1 THEN v_origin_job.mapping_snapshot#>>'{tabs,0,range}'
            ELSE v_job.mapping_snapshot#>>'{tabs,0,range}' END;
          IF v_range IS NULL OR plugin_data.csf_has_edge_padding(v_range) THEN
            v_range_valid := false;
            EXIT;
          END IF;
          v_qualifier := NULL;
          v_parts := regexp_match(v_range, '^''((?:[^'']|'''')*)''!(.*)$');
          ...
          ELSE
            ...
            END IF;
          END IF;
          IF v_qualifier='' OR (v_qualifier IS NOT NULL AND v_qualifier IS DISTINCT FROM v_row.sheet_tab_name) THEN
            v_range_valid := false;
            EXIT;
          END IF;
          v_parts := regexp_match(v_block,'^([A-Za-z]{1,3})([1-9][0-9]*):([A-Za-z]{1,3})([1-9][0-9]*)$');
          IF v_parts IS NULL THEN
            v_range_valid := false;
            EXIT;
          END IF;
          IF length(v_parts[2])>8 OR length(v_parts[4])>8 THEN
            v_range_valid := false;
            EXIT;
          END IF;
          IF v_parts[2]::bigint>10000000 OR v_parts[4]::bigint>10000000
            OR v_parts[2]::bigint>v_parts[4]::bigint
            OR length(v_parts[1])>length(v_parts[3])
            OR (length(v_parts[1])=length(v_parts[3]) AND upper(v_parts[1]) COLLATE "C">upper(v_parts[3]) COLLATE "C") THEN
            v_range_valid := false;
            EXIT;
          END IF;
          v_parsed := v_parsed || ARRAY[ARRAY[upper(v_parts[1]),v_parts[2],upper(v_parts[3]),v_parts[4]]];
        END LOOP;
        EXIT WHEN NOT v_range_valid;
        EXIT WHEN v_parsed[1][1] IS DISTINCT FROM v_parsed[2][1]
          OR v_parsed[1][2] IS DISTINCT FROM v_parsed[2][2]
          OR v_parsed[1][3] IS DISTINCT FROM v_parsed[2][3]
          OR v_parsed[2][4]::bigint<v_parsed[1][4]::bigint
          OR v_row.row_number>v_parsed[1][4]::bigint;
      END IF;
