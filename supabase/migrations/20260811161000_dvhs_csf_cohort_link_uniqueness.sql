...
--
-- `csf_mutate_onboarding_link` deduplicated only by request identifier, so two
-- officers (or one officer whose response was lost and who then retried with a
-- fresh request id) could leave two active `invitation_scope = 'cohort'` links
-- pointing at the same organization, class, and semester. Students then
-- receive different codes for the same class, and deactivating the link an
-- officer can see does not close the class.
--
-- Enforcement has two layers. The RPC refuses the duplicate under the
-- per-organization advisory transaction lock that every create/activate
-- already takes, so an officer gets a sentence they can act on. A partial
-- UNIQUE index makes the same rule true for any write path, including a future
-- caller or a direct fixture insert that never reaches the RPC.
--
-- Direct student-specific invitations keep their own semantics: every check
-- below and the index predicate are scoped to `invitation_scope = 'cohort'`.
-- Receipt replay is also untouched: the request-receipt lookup still runs
-- before any duplicate evaluation, so an exact retry of a committed create
-- still returns its original result instead of colliding with the link it
-- created.

BEGIN;

-- A draft of this migration created a non-unique index under the old name.
-- Drop it so a stack that applied that draft ends in the same state as a clean
-- replay. Nothing outside this unshipped change ever referenced it.
DROP INDEX IF EXISTS plugin_data.csf_onboarding_links_active_cohort_idx;

-- Preflight: fail loudly, never rewrite. A duplicate here is real chapter
-- state, and the only correct resolution is an officer deactivating the extra
-- link through the product's own action, which preserves the link row, its
-- code, and its audit history. Deleting or merging rows inside a migration
-- would destroy invitation provenance and silently invalidate codes students
-- already hold.
DO $$
DECLARE
  v_conflicting_groups integer;
  v_conflicting_links integer;
  v_examples text;
BEGIN
  SELECT
    count(*)::integer,
    coalesce(sum(conflicts.link_count), 0)::integer,
    string_agg(conflicts.description, '; ' ORDER BY conflicts.description)
  INTO v_conflicting_groups, v_conflicting_links, v_examples
  FROM (
    SELECT
      count(*)::integer AS link_count,
      format(
        'organization %s / class %s / semester %s holds %s active links (%s)',
        link.organization_id,
        link.cohort_id,
        link.term_id,
        count(*),
        string_agg(link.code, ', ' ORDER BY link.code)
      ) AS description
    FROM plugin_data.csf_onboarding_links AS link
    WHERE link.invitation_scope = 'cohort'
      AND link.is_active
      AND link.cohort_id IS NOT NULL
      AND link.term_id IS NOT NULL
    GROUP BY link.organization_id, link.cohort_id, link.term_id
    HAVING count(*) > 1
  ) AS conflicts;

  IF v_conflicting_groups > 0 THEN
    RAISE EXCEPTION
      'CSF class invitations: % class/semester pair(s) already hold % active reusable links, so the one-active-link-per-class-and-semester index cannot be created. Deactivate the extra links through Members -> Account connections and re-run this migration. Nothing was deleted or merged. Conflicts: %',
      v_conflicting_groups,
      v_conflicting_links,
      v_examples
      USING ERRCODE = '23505';
  END IF;
...
CREATE UNIQUE INDEX IF NOT EXISTS csf_onboarding_links_active_cohort_uidx
  ON plugin_data.csf_onboarding_links (organization_id, cohort_id, term_id)
...
DECLARE
  ...
  v_active_conflict_code text;
  v_violated_constraint text;
  v_before jsonb;
  v_result jsonb;
  v_duplicate_active_message constant text :=
...
BEGIN
  ...
  -- Every operation starts with the same tenant lock. It protects the request
  -- receipt, deterministic tenant-local invitation-code creation, and the
  -- one-active-link-per-class-and-semester check below, which would otherwise
  -- be a read followed by an unsynchronized write.
  ...
  IF v_operation = 'create' THEN
    ...
    SELECT existing_link.code
    INTO v_active_conflict_code
    FROM plugin_data.csf_onboarding_links AS existing_link
    WHERE existing_link.organization_id = p_organization_id
      AND existing_link.invitation_scope = 'cohort'
      AND existing_link.cohort_id = v_cohort_id
      AND existing_link.term_id = v_term_id
      AND existing_link.is_active
    LIMIT 1;
    IF FOUND THEN
      RAISE EXCEPTION USING
        MESSAGE = v_duplicate_active_message,
        DETAIL = pg_catalog.format('Active class invitation code: %s', v_active_conflict_code),
        ERRCODE = '23505';
    END IF;
    ...
    EXCEPTION WHEN unique_violation THEN
      -- Both uniqueness rules land here. Report the one that actually fired so
      -- an officer is not told to change the code when the real problem is a
      -- second active link for the same class and semester.
      GET STACKED DIAGNOSTICS v_violated_constraint = CONSTRAINT_NAME;
      IF v_violated_constraint = 'csf_onboarding_links_active_cohort_uidx' THEN
        RAISE EXCEPTION USING
          MESSAGE = v_duplicate_active_message,
          ERRCODE = '23505';
      END IF;
  ...
  ELSE
    ...
    IF v_is_active THEN
      ...
      WHERE existing_link.organization_id = p_organization_id
        ...
        AND existing_link.term_id = v_term_id
        AND existing_link.is_active
        AND existing_link.id <> v_link.id
      LIMIT 1;
      IF FOUND THEN
        RAISE EXCEPTION USING
          MESSAGE = v_duplicate_active_message,
...
COMMENT ON FUNCTION plugin_data.csf_mutate_onboarding_link(uuid, uuid, uuid, jsonb) IS
  'Service-only, permission-checked, replay-safe reusable class-link create/activate/deactivate operation that resolves only existing tenant classes, semesters, and active class-semester links, refuses a second active cohort link for the same class and semester under the organization advisory lock and the csf_onboarding_links_active_cohort_uidx partial unique index, and commits its immutable audit receipt atomically.';

COMMENT ON INDEX plugin_data.csf_onboarding_links_active_cohort_uidx IS
  'Enforces one active reusable class invitation per organization, class, and semester, and supports the same check in csf_mutate_onboarding_link. Student-specific direct invitations are out of scope.';
