-- Count an attendance-window exclusion only when every recorded row error is
-- one of the two reviewed cutoff errors. A row that also carries malformed
-- timestamp, identity, or any other error must keep blocking the manual gate.
...
COMMENT ON FUNCTION plugin_data.csf_import_preview_readiness(uuid, uuid) IS
  'Returns service-only whole-preview counts, including only rows whose complete error set consists of reviewed meeting attendance cutoff exclusions, and the durable commit state.';
