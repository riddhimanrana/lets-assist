-- Distinguish settled attendance rows and deterministic import refusals.
BEGIN;
    ...
    SELECT
      ...
      pg_catalog.count(*) FILTER (
        WHERE preview.source_type = 'meeting_attendance'
          AND import_row.import_status = 'duplicate'
          AND import_row.errors = ARRAY['Attendance already exists and was not overwritten.']::text[]
          AND import_row.matched_profile_id IS NOT NULL
      ) AS already_recorded,
      pg_catalog.count(*) FILTER (
        WHERE preview.source_type = 'meeting_attendance'
          AND import_row.import_status = 'error'
          AND pg_catalog.cardinality(import_row.errors) > 0
          AND NOT EXISTS (
            SELECT 1
            FROM pg_catalog.unnest(import_row.errors) AS row_error(message)
            WHERE row_error.message IS NULL
              OR row_error.message NOT IN (
...
CREATE OR REPLACE FUNCTION plugin_data.csf_import_application_response_row_identity_base(
  p_organization_id uuid,
  p_profile_id uuid,
  p_first_name text,
  p_last_name text,
...
DECLARE
  ...
  v_source_snapshot_unchanged boolean := false;
  v_officer_managed boolean := false;
  v_canonical_write_skipped boolean := false;
...
BEGIN
  ...
  WHERE profile.organization_id = p_organization_id
    ...
    AND (
      (profile.school_email IS NULL AND nullif(btrim(p_school_email), '') IS NOT NULL)
      OR (profile.personal_email IS NULL AND nullif(btrim(p_personal_email), '') IS NOT NULL)
      OR profile.source_summary->>'lastApplicationRowHash' IS DISTINCT FROM p_row_hash
    );
  ...
  IF FOUND THEN
    v_source_snapshot_unchanged :=
      v_existing_application.source IN ('google_form_sheet', 'sheet', 'legacy_import')
      AND v_existing_application.cohort_id = p_cohort_id
      AND v_existing_application.profile_id = v_profile_id
      AND v_existing_application.term_id = p_term_id
      AND v_existing_application.source_file_id IS NOT DISTINCT FROM v_import_job.source_file_id
      AND v_existing_application.application_data->>'rowHash' IS NOT DISTINCT FROM p_row_hash
      AND v_existing_application.application_data->'normalizedImport' IS NOT DISTINCT FROM p_application_data;
    v_officer_managed :=
      v_existing_application.source NOT IN ('google_form_sheet', 'sheet', 'legacy_import')
    ...
    IF v_officer_managed AND NOT v_source_snapshot_unchanged THEN
      RAISE EXCEPTION 'A reviewed or officer-managed application already exists and was not overwritten.'
        USING ERRCODE = '23514', DETAIL = 'CSF_IMPORT_REFUSAL=officer_managed_application';
    END IF;

    v_application_id := v_existing_application.id;
    v_import_status := 'updated';
    v_canonical_write_skipped :=
      v_source_snapshot_unchanged
      AND (v_officer_managed OR (
      v_existing_application.source = 'google_form_sheet'
      AND v_existing_application.source_url IS NOT DISTINCT FROM v_source.sheet_url
      AND v_existing_application.source_submitted_at IS NOT DISTINCT FROM v_source_submitted_at
      AND v_existing_application.status = v_legacy_status
      AND v_existing_application.submission_status = v_submission_status
      AND v_existing_application.current_grade_level IS NOT DISTINCT FROM v_current_grade_level
      AND v_existing_application.returning_status IS NOT DISTINCT FROM v_returning_status
      AND v_existing_application.most_checked_email IS NOT DISTINCT FROM v_most_checked_email
      AND v_existing_application.list_i_points IS NOT DISTINCT FROM v_list_i_points
      AND v_existing_application.list_i_ii_points IS NOT DISTINCT FROM v_list_i_ii_points
      AND v_existing_application.grand_total_points IS NOT DISTINCT FROM v_grand_total_points
      AND v_existing_application.source_file_id IS NOT DISTINCT FROM v_import_job.source_file_id
      AND v_existing_application.application_data->>'rowHash' IS NOT DISTINCT FROM p_row_hash
      AND v_existing_application.application_data->'normalizedImport' IS NOT DISTINCT FROM p_application_data
      AND coalesce((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'rawLine', course.raw_line
        ) ORDER BY course.course_list, course.course_name, course.grade, course.points, course.is_bonus, course.raw_line)
        FROM plugin_data.csf_application_course_entries AS course
        WHERE course.organization_id = p_organization_id
          AND course.application_id = v_application_id
      ), '[]'::jsonb) IS NOT DISTINCT FROM coalesce((
        SELECT jsonb_agg(jsonb_build_object(
          'courseList', item.value->>'courseList',
          'courseName', btrim(item.value->>'courseName'),
          'grade', nullif(btrim(item.value->>'grade'), ''),
          'points', nullif(item.value->>'points', '')::numeric,
          'isBonus', coalesce((item.value->>'isBonus')::boolean, false),
          'rawLine', nullif(item.value->>'rawLine', '')
        ) ORDER BY item.value->>'courseList', btrim(item.value->>'courseName'),
          nullif(btrim(item.value->>'grade'), ''), nullif(item.value->>'points', '')::numeric,
          coalesce((item.value->>'isBonus')::boolean, false), nullif(item.value->>'rawLine', ''))
        FROM jsonb_array_elements(coalesce(p_application_data->'courses', '[]'::jsonb)) AS item(value)
      ), '[]'::jsonb)
      AND (
        SELECT count(*)
        FROM plugin_data.csf_application_files AS file
        WHERE file.organization_id = p_organization_id
          AND file.application_id = v_application_id
          AND file.provider = 'google_drive'
          AND file.verification_status = 'unreviewed'
          AND file.file_type IN ('transcript', 'webstore_receipt')
      ) = (CASE WHEN v_transcript_file_id IS NULL THEN 0 ELSE 1 END)
        + (CASE WHEN v_receipt_file_id IS NULL THEN 0 ELSE 1 END)
      AND (v_transcript_file_id IS NULL OR EXISTS (
        SELECT 1 FROM plugin_data.csf_application_files AS file
        WHERE file.organization_id = p_organization_id
          AND file.application_id = v_application_id
          AND file.profile_id = v_profile_id
          AND file.term_id = p_term_id
          AND file.file_type = 'transcript'
          AND file.provider = 'google_drive'
          AND file.verification_status = 'unreviewed'
          AND file.drive_file_id = v_transcript_file_id
          AND file.source_url IS NOT DISTINCT FROM v_transcript_url
      ))
      AND (v_receipt_file_id IS NULL OR EXISTS (
        SELECT 1 FROM plugin_data.csf_application_files AS file
        WHERE file.organization_id = p_organization_id
          AND file.application_id = v_application_id
          AND file.profile_id = v_profile_id
          ...
          AND file.provider = 'google_drive'
          AND file.verification_status = 'unreviewed'
          AND file.drive_file_id = v_receipt_file_id
          AND file.source_url IS NOT DISTINCT FROM v_receipt_url
      ))));
    IF v_canonical_write_skipped THEN
      v_submission_status := v_existing_application.submission_status;
      v_legacy_status := v_existing_application.status;
    END IF;

    IF NOT v_canonical_write_skipped THEN
  ...
  IF NOT v_canonical_write_skipped AND v_transcript_file_id IS NOT NULL THEN
  ...
  IF NOT v_canonical_write_skipped AND v_receipt_file_id IS NOT NULL THEN
    INSERT INTO plugin_data.csf_application_files (
      organization_id, application_id, profile_id, term_id, file_type,
      bucket, object_path, original_filename, uploaded_by,
      provider, drive_file_id, drive_file_name, source_url, verification_status
  ...
  INSERT INTO plugin_data.csf_admin_audit_events (
    ...
    jsonb_build_object(
      ...
      ,'canonicalWriteSkipped', v_canonical_write_skipped
    )
  );
...
GRANT EXECUTE ON FUNCTION plugin_data.csf_import_application_response_row_identity_base(
  uuid, uuid, text, text, text, text, text, text, text, text,
  uuid, uuid, uuid, uuid, text, jsonb, uuid
) TO postgres;


-- Read the immutable preview identity, not the source's current mapping.
CREATE OR REPLACE FUNCTION plugin_data.csf_meeting_preview_id(
  p_organization_id uuid, p_meeting_id uuid, p_preview_job_id uuid DEFAULT NULL
) RETURNS uuid LANGUAGE sql STABLE SECURITY INVOKER SET search_path = '' AS $$
  SELECT j.id FROM plugin_data.csf_sheet_import_jobs j
  WHERE j.organization_id = p_organization_id AND j.mode = 'preview'
    AND j.source_type = 'meeting_attendance'
    AND (p_preview_job_id IS NULL OR j.id = p_preview_job_id)
    AND EXISTS (SELECT 1 FROM plugin_data.csf_term_meetings m
      WHERE m.id = p_meeting_id AND m.organization_id = p_organization_id)
    AND (
      j.mapping_snapshot->>'meetingId' = p_meeting_id::text
      OR (j.mapping_snapshot->>'meetingId' IS NULL
        AND EXISTS (SELECT 1 FROM plugin_data.csf_sheet_import_rows r
          WHERE r.job_id = j.id AND r.organization_id = p_organization_id)
        AND NOT EXISTS (SELECT 1 FROM plugin_data.csf_sheet_import_rows r
          WHERE r.job_id = j.id AND r.organization_id = p_organization_id
            AND (r.normalized_data->>'meetingId') IS DISTINCT FROM p_meeting_id::text))
    )
  ORDER BY j.created_at DESC, j.id DESC LIMIT 1;
$$;
REVOKE ALL ON FUNCTION plugin_data.csf_meeting_preview_id(uuid,uuid,uuid) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_meeting_preview_id(uuid,uuid,uuid) TO service_role;

CREATE OR REPLACE FUNCTION plugin_data.csf_meeting_attendance_counts(
  p_organization_id uuid, p_meeting_ids uuid[]
) RETURNS TABLE(meeting_id uuid, attended_count bigint)
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = '' AS $$
  SELECT m.id, count(a.id) FROM plugin_data.csf_term_meetings m
  LEFT JOIN plugin_data.csf_meeting_attendance a
    ON a.organization_id = m.organization_id AND a.term_id = m.term_id
      AND a.meeting_key = m.meeting_key AND a.status = 'attended'
  WHERE m.organization_id = p_organization_id AND m.id = ANY(p_meeting_ids)
  GROUP BY m.id;
$$;
REVOKE ALL ON FUNCTION plugin_data.csf_meeting_attendance_counts(uuid,uuid[]) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_meeting_attendance_counts(uuid,uuid[]) TO service_role;
