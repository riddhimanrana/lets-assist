-- Add replay-safe editing for the shared CSF semester application window.
BEGIN;
...
COMMENT ON FUNCTION plugin_data.csf_update_cohort_term(uuid, uuid, jsonb, uuid)
  IS 'Service-only, permission-checked, replay-safe atomic edit of one shared semester, its application window, and its exact class-semester link with immutable audit history.';
