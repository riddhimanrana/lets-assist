-- Allow a corrected rule-driven submission to use the category resolved from
-- its selected earning component. Legacy fixed activities still require the
-- activity's stored point type.
...
ALTER FUNCTION plugin_data.csf_resubmit_point_submission(
...
COMMENT ON FUNCTION plugin_data.csf_resubmit_point_submission(
  uuid, uuid, numeric, text, date, text, uuid, uuid
) IS 'Internal member point-resubmission transition. Rule-driven categories come from the selected earning component.';
