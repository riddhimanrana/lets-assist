-- Keep archived class records available through explicit class review.
-- Exclude them from the general directory before counts and pagination.
...
AS $$
  ...
  base AS (
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY (membership.status = 'active') DESC, membership.created_at DESC, membership.id DESC
      LIMIT 1
    ...
    WHERE profile.organization_id = p_organization_id
      ...
      AND (p_cohort_id IS NOT NULL OR cohort.status IS DISTINCT FROM 'archived')
  ),
  enriched AS (
    SELECT
      base.*,
