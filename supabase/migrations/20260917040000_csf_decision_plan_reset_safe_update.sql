-- The two decision RPCs reset their per-call plan tables with an unqualified
-- DELETE. Under the safe-update guard the request role runs with, an
-- unqualified DELETE is rejected before it executes, so
-- csf_stage_sheet_application_decisions fails with "DELETE requires a WHERE
-- clause" when it is called the way the product calls it. pgTAP and direct
-- psql did not see this because neither runs under that session policy.
--
-- pg_temp.csf_decision_source_plan, pg_temp.csf_decision_row_plan and
-- pg_temp.csf_decision_release_plan are session-local scratch tables that each
-- call rebuilds from its arguments, so emptying them completely is the intent.
-- TRUNCATE says that directly and carries no WHERE clause to omit.
--
-- Nothing else moves. Both functions are restated verbatim from their current
-- definitions, 20260917030100 for the stage and 20260917030000 for the
-- release, with only those three statements replaced, and their REVOKE/GRANT
-- restated unchanged. No guard is weakened and no session setting is altered.
