-- Staging, release, and the read projections for Sheets application review.
--
-- Provenance is verified here, in the database, against
-- `csf_sheet_sources` → `csf_sheet_import_jobs` → `csf_sheet_import_rows` →
-- `csf_term_applications`. A caller-supplied application id or match basis is a
-- claim, not evidence: a row whose chain does not hold is recorded as a
-- conflict and stages nothing.

BEGIN;

-- ---------------------------------------------------------------------------
-- A. Staging one sync run
--
-- One transaction: verify every row's provenance, record the immutable run,
-- its per-source evidence, and its per-row evidence, refresh the private
-- stages, and immediately re-apply to rows that were already released.
-- Idempotent on `p_run_id`.
-- ---------------------------------------------------------------------------

CREATE FUNCTION plugin_data.csf_stage_sheet_application_decisions(
...
BEGIN
  ...
  DELETE FROM pg_temp.csf_decision_source_plan;
  ...
  DELETE FROM pg_temp.csf_decision_row_plan;
...
-- ---------------------------------------------------------------------------
-- B. The run receipt, rebuilt from stored evidence
--
-- Counts come from the database's own rows, never from caller arithmetic, so a
-- replay and a first call return the same numbers.
-- ---------------------------------------------------------------------------

CREATE FUNCTION plugin_data.csf_sheet_application_decision_run_receipt(
  p_organization_id uuid,
  p_run_id uuid
)
RETURNS jsonb
...
AS $$
  SELECT pg_catalog.jsonb_build_object(
    ...
    'rows', coalesce((
      ...
      WHERE evidence_row.run_id = run.id
    ), '[]'::jsonb)
  )
  FROM plugin_data.csf_application_decision_sync_runs AS run
  WHERE run.organization_id = p_organization_id
...
-- ---------------------------------------------------------------------------
-- C. A failed read is evidence too
-- ---------------------------------------------------------------------------

CREATE FUNCTION plugin_data.csf_record_sheet_decision_sync_failure(
  p_organization_id uuid,
  p_actor_user_id uuid,
  p_term_id uuid,
  p_run_id uuid,
...
BEGIN
  ...
  IF p_run_id IS NULL OR v_error IS NULL THEN
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'A failed decision sync needs a run identifier and an error.';
  END IF;
  ...
  v_fingerprint := pg_catalog.encode(
    extensions.digest(
      pg_catalog.convert_to(
        pg_catalog.jsonb_build_object(
          'termId', p_term_id, 'evidence', p_evidence, 'error', v_error
        )::text,
        'UTF8'
      ),
      'sha256'
  ...
  SELECT run.* INTO v_existing
  FROM plugin_data.csf_application_decision_sync_runs AS run
  WHERE run.organization_id = p_organization_id AND run.request_id = p_run_id;
  IF FOUND THEN
    IF v_existing.term_id IS DISTINCT FROM p_term_id
      OR v_existing.request_fingerprint IS DISTINCT FROM v_fingerprint THEN
      RAISE EXCEPTION USING
        ...
        HINT = 'CSF_RELOAD_REQUIRED=true';
    END IF;
    RETURN plugin_data.csf_sheet_application_decision_run_receipt(p_organization_id, v_existing.id)
      || pg_catalog.jsonb_build_object('replay', true);
  END IF;
  ...
  SELECT
    p_organization_id, v_run_id, (entry.value ->> 'sourceId')::uuid,
    coalesce(entry.value ->> 'readStatus', 'unavailable'),
    nullif(pg_catalog.btrim(coalesce(entry.value ->> 'message', '')), ''),
    entry.value ->> 'spreadsheetFileId', entry.value ->> 'spreadsheetTitle',
    entry.value ->> 'providerVersion',
    coalesce(entry.value ->> 'sheetTabName', ''),
    (entry.value ->> 'sheetTabId')::bigint,
    coalesce(entry.value ->> 'requestedRange', ''),
    entry.value ->> 'contentHash', entry.value ->> 'mappingVersion'
  FROM pg_catalog.jsonb_array_elements(
    CASE WHEN pg_catalog.jsonb_typeof(coalesce(p_evidence, 'null'::jsonb)) = 'array'
...
REVOKE ALL ON FUNCTION plugin_data.csf_record_sheet_decision_sync_failure(uuid, uuid, uuid, uuid, jsonb, text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION plugin_data.csf_record_sheet_decision_sync_failure(uuid, uuid, uuid, uuid, jsonb, text)
  TO service_role;
