...
-- Match annotation-only error rows without clearing their annotation blocker.
...
BEGIN
  ...
  v_annotation_only_error := v_row.import_status = 'error'
    AND coalesce(cardinality(v_row.errors), 0) > 0
    AND NOT EXISTS (
      SELECT 1 FROM unnest(v_row.errors) AS error_message
      WHERE error_message IS NULL OR error_message NOT LIKE
        'Activity values without explicit numeric points%'
    );
  ...
  IF p_decision = 'match' THEN
    ...
    IF v_row.resolution_status = 'resolved'
      AND (v_row.import_status = 'pending' OR v_annotation_only_error)
    ...
    IF v_row.import_status NOT IN ('ambiguous', 'conflict', 'duplicate') AND NOT (
      (v_row.import_status = 'pending' OR v_annotation_only_error)
      ...
      AND (coalesce(cardinality(v_row.errors),0) = 0 OR v_annotation_only_error)
      AND NOT EXISTS (
    ...
    SET
      ...
      import_status = CASE WHEN v_annotation_only_error THEN 'error' ELSE 'pending' END,
      errors = CASE WHEN v_annotation_only_error THEN v_row.errors ELSE ARRAY[]::text[] END,
