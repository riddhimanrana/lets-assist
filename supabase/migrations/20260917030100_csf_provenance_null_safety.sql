-- An unknown provenance check must fail closed.
--
-- The verification expression compared columns that can be NULL. When an import
-- row belonged to a different applicant and the claimed application had no
-- recorded import row, `matched_application_id = application.id` was false and
-- `source_import_row_id = import_row.id` was NULL, so the whole conjunction
-- evaluated to NULL rather than false. `NOT provenance_verified` on a NULL is
-- NULL, so the fail-closed branch never fired and the row fell through to
-- `changed` and was staged.
--
-- Replay B only caught it because the same batch also claimed that application
-- twice, so `ambiguous_match` masked it. On its own, a sheet row could have
-- handed a verdict to an application it had no lineage to, which is exactly
-- what this verification exists to prevent.
--
-- The fix is `coalesce(..., false)` around the expression: unknown is refused.
...
COMMENT ON FUNCTION plugin_data.csf_stage_sheet_application_decisions(uuid, uuid, uuid, uuid, jsonb, jsonb) IS
  'Stages one decision sync run. A provenance check that cannot be evaluated is refused, not skipped.';
