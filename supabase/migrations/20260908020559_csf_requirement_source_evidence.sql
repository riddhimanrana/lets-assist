-- Expand the preview evidence contract without changing approval or commit payloads.
-- Legacy evidence keeps its existing digest. Workers remain the rollback control.
      ...
      IF v_normalized ? 'sourceEvidence' THEN
        ...
        IF EXISTS (
          ...
          WHERE field NOT IN (
            ...
            'sourceEvidenceHash', 'requirement'
          )
        ) THEN
        ...
        LOOP
          ...
          END IF;
        END LOOP;
        IF v_evidence ? 'requirement' THEN
          v_entry := v_evidence -> 'requirement';
          IF jsonb_typeof(v_entry) IS DISTINCT FROM 'object' THEN
            RAISE EXCEPTION 'CSF requirement evidence must be a bounded object.'
              USING ERRCODE = '23514';
          END IF;
          IF NOT (v_entry ?& ARRAY['columnNumber', 'columnLabel', 'value', 'valueOrigin', 'purpose'])
            OR EXISTS (
              SELECT 1 FROM jsonb_object_keys(v_entry) AS field
              WHERE field NOT IN ('columnNumber', 'columnLabel', 'value', 'valueOrigin', 'purpose')
            )
            OR jsonb_typeof(v_entry -> 'columnNumber') IS DISTINCT FROM 'number'
            OR coalesce(v_entry ->> 'columnNumber', '') !~ '^[1-9][0-9]{0,2}$'
            OR jsonb_typeof(v_entry -> 'columnLabel') IS DISTINCT FROM 'string'
            OR length(v_entry ->> 'columnLabel') NOT BETWEEN 1 AND 160
            OR jsonb_typeof(v_entry -> 'value') IS DISTINCT FROM 'string'
            OR length(v_entry ->> 'value') > 80
            OR coalesce(v_entry ->> 'valueOrigin', '') NOT IN ('formula_result', 'literal')
            OR coalesce(v_entry ->> 'purpose', '') <> 'historical_requirement_evidence'
            OR (v_entry ->> 'valueOrigin' = 'formula_result'
              AND trim(v_entry ->> 'value') !~* '^(x|true|false|yes|no|1|0)?$')
          THEN
            RAISE EXCEPTION 'CSF requirement evidence has an invalid field or completion marker.'
              USING ERRCODE = '23514';
          END IF;
        END IF;
        ...
        IF v_evidence ? 'requirement' THEN
          v_evidence_body := v_evidence_body
            || jsonb_build_object('requirement', v_evidence -> 'requirement');
        END IF;
