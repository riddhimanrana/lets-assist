-- Recover reviewed application identities from immutable retry evidence, not caller targets.
CREATE FUNCTION plugin_data.csf_recover_application_retry_matches(
    ...
    FOR v_depth IN 1..32 LOOP
      ...
      SELECT * INTO v_origin_job FROM plugin_data.csf_sheet_import_jobs
        WHERE organization_id=p_organization_id AND id=v_parent.job_id
          ...
          AND mapping_version=v_job.mapping_version;
      EXIT WHEN NOT FOUND;
