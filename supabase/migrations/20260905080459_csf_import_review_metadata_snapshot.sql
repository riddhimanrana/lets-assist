...
ALTER TABLE plugin_data.csf_sheet_import_rows
  ...
  CONSTRAINT csf_import_rows_resolution_metadata_object
  CHECK (jsonb_typeof(resolution_metadata) = 'object');

COMMENT ON COLUMN plugin_data.csf_sheet_import_rows.resolution_metadata IS
  'Officer-only reconciliation metadata. Separate from immutable source evidence; recorded in the resolution audit.';
...
REVOKE ALL ON FUNCTION plugin_data.csf_reconcile_sheet_import_row_identity_base(uuid, uuid, uuid, text, text, uuid, uuid, jsonb) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION plugin_data.csf_reconcile_sheet_import_row_identity_base(uuid, uuid, uuid, text, text, uuid, uuid, jsonb) TO postgres;
