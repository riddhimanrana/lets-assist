-- Avoid rewriting unchanged application snapshots during automatic full imports.
-- Each preview row and audit event still records its own immutable provenance.

BEGIN;

-- The body below replaces the current identity-base implementation. The
-- public wrapper retains its existing authorization and lock ordering.
