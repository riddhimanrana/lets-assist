...
for (const [name, file] of Object.entries(map)) {
  ...
  const isDefault = new RegExp(`export default (?:function )?${name}\\b`).test(
    src,
  );
  if (isDefault) {
    lines.push(
      `export { default as ${name} } from ${JSON.stringify(resolve(file))};`,
    );
  }
}
...
console.log(
  `entry: ${lines.length} export line(s) -> .design-sync/.manual-entry.mjs`,
);
