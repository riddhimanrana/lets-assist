...
const source = readFileSync("scripts/production/verify-csf-target-schema.sql", "utf8");
test("491 pins complete observation functions and holds the destination state", () => {
  ...
  assert.ok(!acceptedCatalogQuery(source, versions.slice(0, 490)).includes("csf_complete_sheet_sync_observation"));
  assert.equal(acceptedCatalogQuery(source, versions.slice(0, 490)), acceptedCatalogQuery(source, versions.slice(0, 489)));
});
test("490 advances only through the reviewed observation migration", () => {
  const result = prepareMigration(process.cwd(), undefined, versions.slice(0, 490));
  assert.equal((result.query.match(/INSERT INTO supabase_migrations.schema_migrations/gu) ?? []).length, 1);
