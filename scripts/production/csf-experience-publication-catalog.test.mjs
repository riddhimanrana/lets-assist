...
const before = JSON.parse(read("./final-schema-671.json"));
const after = JSON.parse(read("./final-schema-672.json"));
const repository = new URL("../../", import.meta.url).pathname;
const versions = expectedVersions(repository).slice(0, 672);
test("signed experience publication preserves the reviewed schema and install boundary", () => {
  assert.deepEqual(after.objects, before.objects);
  assert.equal(versions.at(-1), "20260924050011");
  assert.equal(
    acceptedCatalogQuery("invalid predecessor", versions),
    finalSchemaCatalog(after, versions),
  );
  const sql = read(
    "../../supabase/migrations/20260924050011_publish_dvhs_csf_1_2_78.sql",
  );
  assert.deepEqual(prohibitedDataWrites(sql), []);
  assert.deepEqual(unreviewedDataWrites(sql), []);
});
test("experience publication is applied once with its catalog fence", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 671),
  );
  assert.match(prepared.query, /'20260924050011','publish_dvhs_csf_1_2_78'/u);
  assert.match(
    prepared.query,
  ...
  assert.doesNotMatch(
    retry.query,
    /'20260924050011','publish_dvhs_csf_1_2_78'/u,
  );
});
