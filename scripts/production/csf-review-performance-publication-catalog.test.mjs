...
const before = JSON.parse(read("./final-schema-648.json"));
const after = JSON.parse(read("./final-schema-649.json"));
const versions = expectedVersions(repository).slice(0, 649);
const name = "20260922054001_publish_dvhs_csf_1_2_67";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF Applications performance release publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 649);
  assert.equal(versions.at(-1), "20260922054001");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 648)));
  assert.throws(
...
test("an accepted 648 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 648),
  );
  assert.equal(prepared.prefix.length, 648);
  assert.match(prepared.query, /'20260922054001','publish_dvhs_csf_1_2_67'/u);
  assert.ok(
