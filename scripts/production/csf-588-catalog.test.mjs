...
const source = readFileSync(new URL("./verify-csf-target-schema.sql", import.meta.url), "utf8");
const ledger = expectedVersions(cwd);

test("588 release pins every new migration and its measured schema", () => {
  assert.equal(ledger.length, 588);
  assert.equal(ledger.at(-1), "20260918080000");
  ...
  for (let count = 583; count <= 587; count++)
    assert.equal(acceptedCatalogQuery(source, ledger.slice(0, count)), preceding);
  for (const version of ledger.slice(582)) {
    const file = Object.keys(migrationDigests).find((name) => name.startsWith(`${version}_`));
    ...
    assert.deepEqual(approvedMigrations.find(([name]) => name === file.slice(0, -4)), [file.slice(0, -4), digest]);
  }
});

test("588 release refuses a changed ledger and catalog predecessor", () => {
  const changed = [...ledger];
  changed[587] = "20990101000000";
  assert.throws(() => acceptedCatalogQuery(source, changed), /explicit release review/u);
  assert.throws(() => acceptedCatalogQuery(source.replace("WHEN (SELECT valid FROM table_posture)", "WHEN true"), ledger), /accepted catalog|contract changed/u);
});
