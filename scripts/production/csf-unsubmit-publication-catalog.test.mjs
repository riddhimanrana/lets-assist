...
test("signed Unsubmit publication preserves the reviewed schema and install boundary", () => {
  ...
  assert.equal(versions.at(-1), "20260924083054");
  assert.equal(
  ...
  const sql = read(
    "../../supabase/migrations/20260924083054_publish_dvhs_csf_1_2_79.sql",
  );
  assert.deepEqual(prohibitedDataWrites(sql), []);
  assert.deepEqual(unreviewedDataWrites(sql), []);
});
test("Unsubmit publication is applied once with its catalog fence", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 673),
  );
  assert.match(prepared.query, /'20260924083054','publish_dvhs_csf_1_2_79'/u);
  assert.match(
    prepared.query,
  ...
  assert.doesNotMatch(
    retry.query,
    /'20260924083054','publish_dvhs_csf_1_2_79'/u,
  );
});
