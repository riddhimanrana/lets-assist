...
const before = JSON.parse(read("./final-schema-655.json"));
const after = JSON.parse(read("./final-schema-656.json"));
const versions = expectedVersions(repository).slice(0, 656);
const name = "20260922181458_publish_dvhs_csf_1_2_72";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF published-review-status publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 656);
  assert.equal(versions.at(-1), "20260922181458");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 655)));
  assert.throws(
...
test("an accepted 655 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 655),
  );
  assert.equal(prepared.prefix.length, 655);
  assert.match(prepared.query, /'20260922181458','publish_dvhs_csf_1_2_72'/u);
  assert.ok(
