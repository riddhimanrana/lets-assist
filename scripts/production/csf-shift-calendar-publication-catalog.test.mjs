...
const before = JSON.parse(read("./final-schema-652.json"));
const after = JSON.parse(read("./final-schema-653.json"));
const versions = expectedVersions(repository).slice(0, 653);
const name = "20260922112314_publish_dvhs_csf_1_2_71";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF shift-calendar publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 653);
  assert.equal(versions.at(-1), "20260922112314");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 652)));
  assert.throws(
...
test("an accepted 652 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 652),
  );
  assert.equal(prepared.prefix.length, 652);
  assert.match(prepared.query, /'20260922112314','publish_dvhs_csf_1_2_71'/u);
  assert.ok(
