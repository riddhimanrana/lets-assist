...
const before = JSON.parse(read("./final-schema-658.json"));
const after = JSON.parse(read("./final-schema-659.json"));
const versions = expectedVersions(repository).slice(0, 659);
const name = "20260923005225_publish_dvhs_csf_1_2_74";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF activity-term-scope publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 659);
  assert.equal(versions.at(-1), "20260923005225");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 658)));
  assert.throws(
...
test("an accepted 658 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 658),
  );
  assert.equal(prepared.prefix.length, 658);
  assert.match(prepared.query, /'20260923005225','publish_dvhs_csf_1_2_74'/u);
  assert.ok(
