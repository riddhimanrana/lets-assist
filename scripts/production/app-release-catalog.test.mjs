...
test("exact 537 release publication preserves the accepted 536 schema", () => {
  const fullLedger = expectedVersions(
    fileURLToPath(new URL("../../", import.meta.url)),
  );
  assert.equal(fullLedger.length, 537);
  assert.equal(fullLedger.at(-1), "20260915195501");
  assert.equal(
    acceptedCatalogQuery(source, fullLedger),
    acceptedCatalogQuery(source, fullLedger.slice(0, 536)),
  );
  assert.equal(
    acceptedCatalogQuery(source, fullLedger.slice(0, 536)),
  ...
  alteredLedger[536] = "20990101000000";
  assert.throws(
...
test("511 changes only the local discussion writer fingerprint", () => {
  ...
  const migration = readFileSync(
    new URL(
      "../../supabase/migrations/20260913200500_csf_sheet_discussion_write_guard.sql",
      import.meta.url,
    ),
    "utf8",
  );
  assert.ok(
    current.includes(
      createHash("md5").update(migration.split("$$")[1]).digest("hex"),
    ),
  );
  assert.ok(current.includes("'bd6c8251c5da723fbc9a1055f8e60a92',true)"));
});
