...
test("staff account connection pins its body and service-only ACL while retaining the preceding catalog", () => {
  ...
  const signature = "plugin_data.csf_staff_connect_profile_account(uuid,uuid,uuid,text,text,uuid)";
  assert.ok(current.includes(signature));
  assert.ok(current.includes(`md5(p.prosrc)='${digest}'`));
  assert.ok(current.includes("p.proargnames=ARRAY['p_organization_id','p_profile_id','p_actor_user_id','p_account_email','p_reason','p_request_id']"));
  assert.ok(current.includes("a.grantee IN ('postgres'::regrole,'service_role'::regrole)"));
