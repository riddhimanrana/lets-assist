...
const before = JSON.parse(read("./final-schema-665.json"));
const after = JSON.parse(read("./final-schema-666.json"));
const versions = expectedVersions(repository).slice(0, 666);
const name = "20260923044405_publish_dvhs_csf_1_2_76";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF point-exception and activity email publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 666);
  assert.equal(versions.at(-1), "20260923044405");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 665)));
  assert.throws(
...
test("an accepted 665 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 665),
  );
  assert.equal(prepared.prefix.length, 665);
  assert.match(prepared.query, /'20260923044405','publish_dvhs_csf_1_2_76'/u);
  assert.ok(
