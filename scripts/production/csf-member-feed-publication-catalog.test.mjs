...
const before = JSON.parse(read("./final-schema-650.json"));
const after = JSON.parse(read("./final-schema-651.json"));
const versions = expectedVersions(repository).slice(0, 651);
const name = "20260922094708_publish_dvhs_csf_1_2_69";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF member-feed publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 651);
  assert.equal(versions.at(-1), "20260922094708");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 650)));
  assert.throws(
...
test("an accepted 650 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 650),
  );
  assert.equal(prepared.prefix.length, 650);
  assert.match(prepared.query, /'20260922094708','publish_dvhs_csf_1_2_69'/u);
  assert.ok(
