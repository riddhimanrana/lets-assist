...
const before = JSON.parse(read("./final-schema-675.json"));
const after = JSON.parse(read("./final-schema-676.json"));
const repository = new URL("../../", import.meta.url).pathname;
const versions = expectedVersions(repository).slice(0, 676);
...
test("experience publication is applied once with its catalog fence", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 675),
  );
  assert.match(prepared.query, /'20260924103737','publish_dvhs_csf_1_2_80'/u);
  assert.match(
    prepared.query,
  ...
  assert.doesNotMatch(
    retry.query,
    /'20260924103737','publish_dvhs_csf_1_2_80'/u,
  );
});
