...
const before = JSON.parse(read("./final-schema-661.json"));
const after = JSON.parse(read("./final-schema-662.json"));
const versions = expectedVersions(repository).slice(0, 662);
const name = "20260923013334_publish_dvhs_csf_1_2_75";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF oauth-submission-review publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 662);
  assert.equal(versions.at(-1), "20260923013334");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 661)));
  assert.throws(
...
test("an accepted 661 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 661),
  );
  assert.equal(prepared.prefix.length, 661);
  assert.match(prepared.query, /'20260923013334','publish_dvhs_csf_1_2_75'/u);
  assert.ok(
