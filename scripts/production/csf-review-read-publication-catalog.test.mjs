...
const before = JSON.parse(read("./final-schema-651.json"));
const after = JSON.parse(read("./final-schema-652.json"));
const versions = expectedVersions(repository).slice(0, 652);
const name = "20260922105553_publish_dvhs_csf_1_2_70";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF review-read publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 652);
  assert.equal(versions.at(-1), "20260922105553");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 651)));
  assert.throws(
...
test("an accepted 651 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 651),
  );
  assert.equal(prepared.prefix.length, 651);
  assert.match(prepared.query, /'20260922105553','publish_dvhs_csf_1_2_70'/u);
  assert.ok(
