import { sheetRecoveryDefinitions, sheetRecoveryTables } from "./sheet-recovery-catalog.mjs";
const overrides = [
  [
...
export const sheetObservationDefinitions = [...sheetRecoveryDefinitions.filter(([name]) => !overrides.some(([signature]) => signature === name)), ...overrides];
export const sheetObservationTables = sheetRecoveryTables.map(row => row[0] === "csf_sheet_sync_destinations" ? [row[0], "d3c32a890093bf3379dfd0e8668c3661", false] : row);
