...
test("492 advances through profile note export deferral and the current release tail", () => {
  const result = prepareMigration(cwd, undefined, versions.slice(0, 492));
  assert.equal(
    (
      result.query.match(
    ...
    expectedVersions(cwd).length - 492,
  );
  assert.ok(
  ...
  assert.deepEqual(
    ...
    [
      ...
      "csf_bind_activity_email_campaign",
      "csf_guard_activity_email_intent_snapshot",
      "csf_guard_activity_email_frozen_recipient",
      "csf_guard_activity_email_queue_transition",
      "csf_point_exception_review_guard",
      "csf_point_exception_credit_guard",
      "csf_activity_catalog_changed",
      "zz_csf_submission_revision",
