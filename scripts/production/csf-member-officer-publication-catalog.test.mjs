...
const before = JSON.parse(read("./final-schema-657.json"));
const after = JSON.parse(read("./final-schema-658.json"));
const versions = expectedVersions(repository).slice(0, 658);
const name = "20260923001620_publish_dvhs_csf_1_2_73";
const sql = read(`../../supabase/migrations/${name}.sql`);

test("CSF member-officer-experience publication preserves the reviewed database schema", () => {
  assert.equal(versions.length, 658);
  assert.equal(versions.at(-1), "20260923001620");
  ...
  assert.throws(() => finalSchemaCatalog(after, versions.slice(0, 657)));
  assert.throws(
...
test("an accepted 657 ledger receives only the new publication", () => {
  const prepared = prepareMigration(
    repository,
    readFileSync,
    versions.slice(0, 657),
  );
  assert.equal(prepared.prefix.length, 657);
  assert.match(prepared.query, /'20260923001620','publish_dvhs_csf_1_2_73'/u);
  assert.ok(
