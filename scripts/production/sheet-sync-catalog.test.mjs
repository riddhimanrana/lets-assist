  ...
  for (const [, name, body] of bodies) {
    ...
    assert.equal(
      ...
      ![
        ...
        "csf_sheet_file_has_other_workspace",
...
test("483 adds the sync posture without changing the preceding accepted catalog", () => {
  assert.equal(versions.length, 511);
  ...
  assert.ok(current.includes("SELECT count(*)=29"));
  assert.throws(
    () =>
...
test("an existing 482 migration release applies sync and signed publications", () => {
...
test("an existing 485 release applies the remaining signed publications and discussion extension", () => {
