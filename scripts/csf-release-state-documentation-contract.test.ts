...
describe("CSF release-state documentation truthfulness guards", () => {
  test("current status separates the repository ledger from hosted database and deployed code", () => {
    ...
    expect(currentState).toContain(
      "repository candidate has 323 ordered migrations through",
    );
    expect(currentState).toContain(
    ...
    expect(currentState).toContain("migration-current at 322 rows through");
    expect(currentState).toContain(
      "Production remains untouched at the audited 236-row baseline",
    );
    expect(currentState).toContain(
      "exact read-only preflight now expects an 87-migration cutover",
    );
    expect(currentState).toContain("fresh exact 323-migration replay");
    expect(currentState).toContain("161 pgTAP files with 5,901 assertions");
  ...
  test("the officer runbook tracks the exact current cutover ledger", () => {
    expect(officerRunbook).toContain(
      "this repository carries 323 ordered migrations through `20260818134000_restate_partner_function_acls`",
    );
    expect(officerRunbook).toContain(
      "exact repository-pinned 87-migration cutover",
    );
    expect(officerRunbook).toContain("ordered ledger through `20260818134000`");
  ...
  test("production cutover baseline tracks the exact pending migration range", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "current repository release candidate has exactly 323 ordered migrations through `20260818134000_restate_partner_function_acls`",
    );
    expect(productionCutoverRunbook).toContain(
      "read-only preflight pins an exact 87-migration tail",
    );
    expect(productionCutoverRunbook).toContain(
    ...
    expect(productionCutoverRunbook).toContain(
      "fresh full 323-migration replay",
    );
  ...
  test("the release pin is exact while hosted and Production gates remain open", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "typed read-only preflight pins an exact 87-migration tail",
    );
    expect(productionCutoverRunbook).toContain(
    ...
    expect(preflight).toContain("repository target 323");
    expect(preflight).toContain("exact 87-row tail");
  });
