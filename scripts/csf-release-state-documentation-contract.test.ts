...
describe("CSF release-state documentation truthfulness guards", () => {
  test("current status separates the repository ledger from hosted database and deployed code", () => {
    ...
    expect(currentState).toContain(
      "repository candidate has 324 ordered migrations through",
    );
    expect(currentState).toContain(
    ...
    expect(currentState).toContain("migration-current at 323 rows through");
    expect(currentState).toContain(
      "Production remains untouched at the audited 236-row baseline",
    );
    expect(currentState).toContain(
      "exact read-only preflight now expects an 88-migration cutover",
    );
    expect(currentState).toContain("fresh exact 324-migration replay");
    expect(currentState).toContain("162 pgTAP files with 5,911 assertions");
  ...
  test("the officer runbook tracks the exact current cutover ledger", () => {
    expect(officerRunbook).toContain(
      "this repository carries 324 ordered migrations through `20260818150000_bound_feedback_candidate_rotation`",
    );
    expect(officerRunbook).toContain(
      "exact repository-pinned 88-migration cutover",
    );
    expect(officerRunbook).toContain("ordered ledger through `20260818150000`");
  ...
  test("production cutover baseline tracks the exact pending migration range", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "current repository release candidate has exactly 324 ordered migrations through `20260818150000_bound_feedback_candidate_rotation`",
    );
    expect(productionCutoverRunbook).toContain(
      "read-only preflight pins an exact 88-migration tail",
    );
    expect(productionCutoverRunbook).toContain(
    ...
    expect(productionCutoverRunbook).toContain(
      "fresh full 324-migration replay",
    );
  ...
  test("the release pin is exact while hosted and Production gates remain open", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "typed read-only preflight pins an exact 88-migration tail",
    );
    expect(productionCutoverRunbook).toContain(
    ...
    expect(preflight).toContain("repository target 324");
    expect(preflight).toContain("exact 88-row tail");
  });
