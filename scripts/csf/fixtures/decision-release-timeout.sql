CREATE FUNCTION pg_temp.fid(text) RETURNS uuid LANGUAGE sql IMMUTABLE AS $$SELECT md5('__RUN_ID__' || $1)::uuid$$;
INSERT INTO auth.users(id,aud,role,email,email_confirmed_at,raw_app_meta_data,raw_user_meta_data) VALUES(pg_temp.fid('officer'),'authenticated','authenticated','release-__RUN_ID__@local.test',now(),'{}','{}');
INSERT INTO public.organizations(id,name,username,type,join_code) VALUES(pg_temp.fid('org'),'Release benchmark','release-__RUN_ID__','school',(100000 + mod(abs(hashtextextended('__RUN_ID__',0)),900000))::text);
...
INSERT INTO plugin_data.csf_sheet_sources(id,organization_id,title,provider,source_type,drive_file_id,spreadsheet_id) VALUES(pg_temp.fid('source'),pg_temp.fid('org'),'Fictional responses','google_sheets','application_responses','fictional-__RUN_ID__-responses','fictional-__RUN_ID__-responses');
...
INSERT INTO plugin_data.csf_sheet_sync_destinations(id,organization_id,spreadsheet_file_id,sheet_id,kind,cohort_id,term_id,is_test,configured_by,enabled,privacy_verified_at,comment_capability,discussion_transport) SELECT pg_temp.fid('destination'||n),pg_temp.fid('org'),'fictional-__RUN_ID__-export-'||n,0,CASE WHEN n=0 THEN 'applications' ELSE 'class' END,CASE WHEN n=0 THEN NULL ELSE pg_temp.fid('cohort'||n) END,pg_temp.fid('term'),true,pg_temp.fid('officer'),true,now(),'available','none' FROM generate_series(0,4)n;
