...
const bootstrapUserIdFile = path.join("/tmp", "lets-assist-bootstrap-user-ids.json");

try {
  if (fs.existsSync(bootstrapUserIdFile)) {
    fs.unlinkSync(bootstrapUserIdFile);
  }
} catch {
  // Best-effort cleanup.
}
...
for (const account of accounts) {
  ...
  execFileSync("node", ["scripts/local-dev/bootstrap-auth-user.mjs"], {
    ...
    env: {
      ...
      BOOTSTRAP_USER_ID_FILE: bootstrapUserIdFile,
    },
  });
}
