...
const fixturePassword =
  process.env.DV_LOCAL_TEST_PASSWORD ?? process.env.CSF_LOCAL_TEST_PASSWORD;
...
const accounts = [
  ...
  },
  {
    ...
    fullName: "DV Staff",
    roles: ["staff", null, null],
  },
  {
    key: "studentA",
    email: "dv.student.a@local.test",
    fullName: "Alex Student",
    roles: ["member", null, null],
  },
  {
    key: "studentB",
    email: "dv.student.b@local.test",
    fullName: "Blair Student",
    roles: ["member", null, null],
  },
  {
    key: "studentC",
...
async function waitForPostgrestSchema(admin) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (attempt === maxAttempts) {
      throw new Error(
        `PostgREST schema cache did not become ready: ${error.message}`,
      );
    }
...
async function main() {
  ...
  await must(
    ...
    admin.from("organizations").upsert({
      ...
      type: "school",
      description:
        "Deterministic local fixture organization for Dougherty Valley Speech & Debate.",
  ...
  await must(
    "seasons",
    plugin.from("org_seasons").upsert(
      [
        {
          id: IDS.priorSeason,
  ...
  await must(
    ...
    admin.from("projects").upsert({
      ...
      verification_method: "manual",
      schedule: {
        oneTime: { date: "2026-12-05", startTime: "09:00", endTime: "13:00" },
      },
      status: "upcoming",
      visibility: "organization_only",
      require_login: true,
  ...
  await must(
    ...
    admin.from("projects").upsert({
      ...
      verification_method: "manual",
      schedule: {
        oneTime: { date: "2026-10-24", startTime: "08:30", endTime: "15:30" },
      },
      status: "upcoming",
      visibility: "organization_only",
      require_login: true,
  ...
  await must(
    "tournament",
    plugin.from("dv_sd_tournaments").upsert({
      id: IDS.tournament,
      organization_id: IDS.organization,
  ...
  const studentByUser = Object.fromEntries(
    studentRows.map((row) => [row.user_id, row.id]),
  );

  await must(
    "households",
    plugin.from("dv_sd_households").upsert([
      {
        id: IDS.household,
        organization_id: IDS.organization,
        display_name: "Student Household",
      },
      {
        id: IDS.secondHousehold,
        organization_id: IDS.organization,
        display_name: "Casey Household",
      },
    ]),
  );
  ...
  await must(
    "guardians",
    plugin.from("dv_sd_guardians").upsert(
      [
        {
          id: IDS.guardian,
          organization_id: IDS.organization,
  ...
  await must(
    "household students",
    plugin.from("dv_sd_household_students").upsert([
      {
  ...
  await must(
    "household guardians",
    plugin.from("dv_sd_household_guardians").upsert([
      {
        household_id: IDS.household,
      ...
      {
        household_id: IDS.secondHousehold,
        guardian_id: IDS.secondGuardian,
        is_primary_contact: true,
      },
    ]),
  );
  ...
  const memberships = await must(
    ...
    plugin
      ...
      .upsert(
        ...
        { onConflict: "organization_id,season_id,student_id" },
      )
      .select("id,student_id,season_id"),
  );
  ...
  await must(
    "requirements",
    plugin.from("dv_sd_membership_requirements").upsert(
      [
        {
          membership_id:
        ...
        {
          membership_id:
            currentMembershipByStudent[studentByUser[users.studentC.id]],
  ...
  const serviceAccounts = await must(
    "service accounts",
    plugin
      .from("dv_sd_family_service_accounts")
      .upsert(
        [
          {
            organization_id: IDS.organization,
        ...
        { onConflict: "organization_id,season_id,household_id" },
      )
  ...
  const registration = await must(
    "registration",
    plugin
      .from("dv_sd_tournament_registrations")
      .upsert(
        {
          organization_id: IDS.organization,
        ...
        { onConflict: "tournament_id,membership_id" },
      )
      .select("id")
      .single(),
  );
  ...
  await must(
    "entry",
    plugin.from("dv_sd_registration_entries").upsert(
      {
  ...
  await must(
    "judge availability",
    plugin.from("dv_sd_judge_availability").upsert(
      {
        organization_id: IDS.organization,
        tournament_id: IDS.tournament,
        judge_id: judge.id,
  ...
  const account = serviceAccounts.find(
    (row) => row.household_id === IDS.household,
  );
  ...
  if (!count) {
    await must(
      "service ledger",
      plugin.from("dv_sd_family_service_ledger").insert({
