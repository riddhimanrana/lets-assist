...
async function upsertDummyUser({ url, serviceRoleKey, email, password, username, fullName }) {
  const lookupUserIdByEmail = async () => {
    try {
      ...
      return execFileSync(
        ...
        [
          ...
          "-At",
          "-c",
          `SELECT id FROM auth.users WHERE lower(email) = lower('${escapedEmail}') LIMIT 1;`,
        ],
        {
          encoding: "utf-8",
          env: {
  ...
  if (!createRes.ok) {
    if (createRes.status === 422) {
      const dbUserId = await lookupUserIdByEmail();
      if (dbUserId) {
        return dbUserId;
      }
    }
...
function readBootstrapUserIds(filePath) {
...
async function main() {
  ...
  const bootstrapUserIds = readBootstrapUserIds(env.BOOTSTRAP_USER_ID_FILE || "/tmp/lets-assist-bootstrap-user-ids.json");
  ...
  for (const user of users) {
    const bootstrapId = bootstrapUserIds[user.email.toLowerCase()];
    const id = bootstrapId || await upsertDummyUser({
      url: supabaseUrl,
      serviceRoleKey,
      email: user.email,
      password,
