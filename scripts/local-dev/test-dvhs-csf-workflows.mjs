...
const anonPlugin = createClient(url, anonKey, {
  auth: { autoRefreshToken: false, persistSession: false },
  db: { schema: "plugin_data" },
});
...
if (orgError || !organization)
  throw orgError ?? new Error("DVHS CSF organization fixture is missing.");
...
const [
  ...
  plugin
    ...
    .select("id, profile_id, term_id, status, application_id")
    .eq("organization_id", organization.id),
  plugin
    .from("csf_terms")
    .select("id, code, is_current, closed_at")
    .eq("organization_id", organization.id),
  plugin
    .from("csf_point_submissions")
    .select("id, profile_id, term_id, status")
    .eq("organization_id", organization.id),
  plugin
    .from("csf_submission_files")
    .select("id, submission_id, bucket, object_path")
    .eq("organization_id", organization.id),
  plugin
    .from("csf_credit_records")
    .select("id, submission_id, profile_id, term_id, points, status")
    .eq("organization_id", organization.id),
  plugin
...
assert(
  (profilesResult.data?.length ?? 0) >= 3,
  "CSF workflow fixture must include multiple profiles.",
);
assert(
  (membershipsResult.data?.length ?? 0) >= 2,
  "CSF workflow fixture must include explicit term memberships.",
);
assert(
  (termsResult.data ?? []).some((term) => term.code === "S26"),
  "S26 legacy term is missing.",
);
assert(
  (auditResult.data?.length ?? 0) > 0,
  "Consequential CSF workflow audit events are missing.",
);
...
for (const file of filesResult.data ?? [])
  fileCounts.set(
    file.submission_id,
    (fileCounts.get(file.submission_id) ?? 0) + 1,
  );
assert(
  [...fileCounts.values()].every((count) => count === 1),
...
assert(
  Boolean(proofProbeSubmission),
  "CSF workflow fixture has no submission for the proof uniqueness probe.",
);
...
for (const credit of creditsResult.data ?? []) {
  ...
  creditCounts.set(
    ...
    (creditCounts.get(credit.submission_id) ?? 0) + 1,
  );
}
assert(
  [...creditCounts.values()].every((count) => count === 1),
  "A point submission produced more than one credit award.",
);
const awardedSubmission = (creditsResult.data ?? []).find(
  (credit) => credit.submission_id,
);
assert(
  Boolean(awardedSubmission),
  "CSF workflow fixture has no awarded submission for the credit uniqueness probe.",
);
...
async function deleteProbeRows(table, ids) {
  ...
  if (result?.error) {
    cleanupFailures.push(
      `${table} probe cleanup failed: ${result.error.message}`,
    );
    return;
  }
  ...
  if (stranded.length > 0) {
    cleanupFailures.push(
      `${table} probe cleanup left ${stranded.join(", ")} in place.`,
    );
  }
}

try {
  const firstProofInsert = await plugin
    .from("csf_submission_files")
    .insert(proofProbe)
    .select("id")
    .single();
  ...
  const duplicateProofInsert = await plugin
    .from("csf_submission_files")
    .insert({
      ...proofProbe,
      object_path: `${proofProbe.object_path}.duplicate`,
    })
    .select("id")
    .single();
  if (duplicateProofInsert.data?.id)
    proofProbeIds.push(duplicateProofInsert.data.id);
  assert(
    duplicateProofInsert.error?.code === "23505",
  ...
  const duplicateCreditInsert = await plugin
    .from("csf_credit_records")
    .insert({
      organization_id: organization.id,
    ...
    .select("id")
    .single();
  if (duplicateCreditInsert.data?.id)
    creditProbeIds.push(duplicateCreditInsert.data.id);
  assert(
    duplicateCreditInsert.error?.code === "23505",
...
if (probeFailure) {
  for (const failure of cleanupFailures)
    console.error(`CSF probe cleanup failure: ${failure}`);
  throw probeFailure;
}
if (cleanupFailures.length > 0) {
  throw new Error(
    `CSF uniqueness probes left attributable rows behind: ${cleanupFailures.join(" | ")}`,
  );
}

const { data: anonymousProfiles, error: anonymousProfileError } =
  await anonPlugin.from("csf_profiles").select("id").limit(1);
assert(
  Boolean(anonymousProfileError) || (anonymousProfiles?.length ?? 0) === 0,
  "Anonymous client could read private CSF profiles.",
);
...
if (appOrigin) {
  const rootResponse = await requestPublicRoute("/organization/dvhs-csf", {
    redirect: "manual",
  });
  assert(
    rootResponse.status === 307,
    `Organization root returned ${rootResponse.status}, expected 307.`,
  );
  assert(
    rootResponse.headers.get("location")?.endsWith("/plugins/dvhs-csf/public"),
    "Organization root did not redirect to the CSF public route.",
  );
  const publicResponse = await requestPublicRoute(
    "/organization/dvhs-csf/plugins/dvhs-csf/public",
  );
  const html = await publicResponse.text();
  assert(
    publicResponse.ok,
    `CSF public route returned ${publicResponse.status}.`,
  );
  assert(
    html.includes("Dougherty Valley High School CSF"),
    "CSF public route is missing the organization identity.",
  );
  assert(
    html.includes("Upcoming activities"),
  ...
  assert(
    !html.includes("Student records stay private"),
    "CSF public route still renders removed privacy-marketing copy.",
  );
  for (const profile of profilesResult.data ?? []) {
    assert(
      !html.includes(`${profile.first_name} ${profile.last_name}`),
      `Public CSF route leaked ${profile.first_name} ${profile.last_name}.`,
    );
    for (const email of [profile.school_email, profile.personal_email]) {
      if (email)
        assert(
          !html.includes(email),
          "Public CSF route leaked a private member email.",
        );
    }
  }
  ...
  for (const identifier of privateIdentifiers) {
    assert(
      !html.includes(String(identifier)),
      "CSF public route serialized a private record identifier.",
    );
  }
...
console.log(
  JSON.stringify(
    {
      ok: true,
      verified: publicRouteVerified
        ? "database-workflows-and-public-route"
        : "database-workflows-only",
