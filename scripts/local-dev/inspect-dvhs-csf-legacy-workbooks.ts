...
import {
  parseCsfSheetRows,
  summarizeCsfParsedRows,
  type CsfSheetCell,
} from "../../lib/plugins/private/plugins/dvhs-csf/services/sheet-import";
...
for (const filePath of files) {
  ...
  if (
    lower.includes("returning club") ||
    lower.includes("club audit") ||
    lower.includes("club point") ||
    lower.includes("clubs point")
  ) {
    const parsed = await parseCsfPartnerFormResponsesWorkbook(
  ...
  if (lower.includes("meeting attendance")) {
    ...
    const parsed = parseCsfMeetingAttendanceRows(
      workbookRows(workbook, sheetName),
    );
    summaries.push({
      file: fileName,
  ...
  if (lower.includes("application")) {
    ...
    summaries.push({
      ...
      kind: "applications",
      sheet: sheetName,
      ...summarizeCsfParsedRows(parsed),
    });
    continue;
  }

  const termTabs = workbook.SheetNames.filter((sheetName) =>
    /^[FS]\d{2}$/i.test(sheetName),
  );
  const tabs = termTabs.map((sheetName) => {
    const parsed = parseCsfSheetRows(
      sheetName,
      workbookRows(workbook, sheetName),
    );
