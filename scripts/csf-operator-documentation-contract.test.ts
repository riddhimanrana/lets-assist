...
describe("CSF operator documentation truthfulness guards", () => {
  ...
  test("current hosted Development status separates database parity from stale deployed code", () => {
    const migrations = readdirSync(join(repositoryRoot, "supabase/migrations"))
      ...
      .sort();
    expect(migrations).toHaveLength(280);
    ...
    expect(currentState).toContain(
      "repository branch has 280 ordered migrations through",
    );
    expect(currentState).toContain(
    ...
    expect(currentState).toContain(
      "seven reviewed migrations behind this repository branch",
    );
    expect(currentState).toContain(
      "Production remains at 236 ordered migrations through `20260811001500`",
    );
    expect(currentState).toContain("44-migration cutover has not run");
    expect(currentState).toContain(
  ...
  test("the Development rehearsal and cutover ledger carry the same current evidence", () => {
    ...
    expect(rehearsalState).toContain(
      "They have not been re-established for either hosted 273 or repository 280",
    );
    expect(rehearsalState).toContain(
    ...
    expect(cutover).toContain(
      "Replay the ordered migration ledger through `20260813010000`",
    );
    expect(cutover).toContain(
    ...
    expect(cutover).toContain("full 44-migration transition");
    expect(cutover).toContain("preflight on the 280-row target");
  });
  ...
  test("production cutover baseline tracks the exact pending migration range", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "Hosted Development Supabase has 273 ordered migrations through `20260812152300`; this repository has 280 through `20260813010000`",
    );
    expect(productionCutoverRunbook).toContain(
    ...
    expect(productionCutoverRunbook).toContain(
      "Production therefore has exactly 44 pending migrations",
    );
    expect(productionCutoverRunbook).toContain(
      "external Vercel 100-deployment-per-day project cap",
    );
