...
describe("CSF operator documentation truthfulness guards", () => {
  ...
  test("current status separates the repository ledger from hosted database and deployed code", () => {
    const migrations = readdirSync(join(repositoryRoot, "supabase/migrations"))
      ...
      .sort();
    expect(migrations).toHaveLength(285);
    ...
    expect(currentState).toContain(
      "repository branch has 285 ordered migrations through",
    );
    expect(currentState).toContain(
    ...
    expect(currentState).toContain("The twelve unmerged migrations are");
    expect(currentState).toContain("`20260813010000_atomic_ai_quota_receipts`");
    expect(currentState).toContain(
    ...
    expect(currentState).toContain("49-migration cutover has not run");
    expect(currentState).toContain(
  ...
  test("the Development rehearsal and cutover ledger carry the same current evidence", () => {
    ...
    expect(rehearsalState).toContain(
      "this repository has 285 through `20260813020000_cancellation_preserves_unknown_delivery_outcomes`",
    );
    expect(rehearsalState).toContain("The twelve unmerged migrations are");
    expect(rehearsalState).toContain(
      "`20260812203500_close_plugin_data_browser_default_acl`",
    );
    expect(rehearsalState).toContain(
    ...
    expect(rehearsalState).toContain(
      "They have not been re-established for either hosted 273 or repository 285",
    );
    expect(rehearsalState).toContain(
    ...
    expect(cutover).toContain("full 49-migration transition");
    expect(cutover).toContain("preflight on the 285-row target");
  });
  ...
  test("production cutover baseline tracks the exact pending migration range", () => {
    ...
    expect(productionCutoverRunbook).toContain(
      "The twelve unmerged migrations have not been applied or deployed in hosted Development",
    );
    expect(productionCutoverRunbook).toContain(
      "`20260812220000_csf_meeting_permission_followups`",
    );
    ...
    expect(productionCutoverRunbook).toContain(
      "Production therefore has exactly 49 pending migrations",
    );
