...
describe("CSF operator documentation truthfulness guards", () => {
  ...
  test("current hosted Development status separates database parity from stale deployed code", () => {
    ...
    expect(migrations.at(-1)).toBe(
      "20260812190442_close_plugin_data_browser_default_acl.sql",
    );
    ...
    expect(currentState).toContain(
      "`20260812190442_close_plugin_data_browser_default_acl`",
    );
    expect(currentState).toContain(
  ...
  test("the Development rehearsal and cutover ledger carry the same current evidence", () => {
    ...
    expect(rehearsalState).toContain(
      "this repository has 275 through `20260812190442_close_plugin_data_browser_default_acl`",
    );
    expect(rehearsalState).toContain(
      "external Vercel 100-deployment-per-day project cap",
    );
    ...
    expect(cutover).toContain(
      "Replay the ordered migration ledger through `20260812190442`",
    );
    expect(cutover).toContain(
